...
Laplace\footnote{Vegeu el capítol \ref{sec:laplace} per a un tractament extens de la transformada de Laplace.}).\index{transformada de Laplace}
...
\begin{exemple}[Teorema de Millman --- Bateries en paraŀlel]\label{ex:mill_bat}
	\addcontentsxms{Teorema de Millman --- Bateries en paraŀlel}
...
\begin{exemple}[Teorema de Millman --- Càrregues trifàsiques amb corrent de neutre]
	\addcontentsxms{Teorema de Millman --- Càrregues trifàsiques amb corrent de neutre}
...
\begin{exemple}[Teorema de Millman --- Càrregues trifàsiques sense corrent de neutre]
	\addcontentsxms{Teorema de Millman --- Càrregues trifàsiques sense corrent de neutre}
...
\begin{exemple}[Aplicació del teorema de la superposició]
	\label{ex:teorema-superpos}
	\addcontentsxms{Aplicació del teorema de la superposició}
...
\begin{exemple}[Càlcul de factors de cresta, de forma i d'arrissada]
	\addcontentsxms{Càlcul de factors de cresta, de forma i d'arrissada}
...
\begin{exemple}[Valor eficaç i valor rectificat mitjà]\label{ex:v-mitj-v-ef}
	\addcontentsxms{Valor eficaç i valor rectificat mitjà}
	Hi ha voltímetres que en lloc de mesurar directament el valor eficaç d'una ona sinusoidal, el que fan és fer una rectificació d'ona completa de l'ona sinusoidal i en calculen el valor mitjà. Quin factor cal aplicar a aquest valor rectificat mitjà per trobar el valor sinusoidal eficaç?
	...
	\[
		...
		\frac{\piup}{2 K} \bar{V}\ped{rec} = 1
	\]
	
	Igualant les dues expressions que són igual a 1, tenim:
	\[
		\frac{\sqrt{2}}{K} V\ped{sin} =  \frac{\piup}{2 K} \bar{V}\ped{rec}
		\quad \Rightarrow \quad 
		V\ped{sin} =  \frac{\piup}{2 \sqrt{2}} \bar{V}\ped{rec} \approx \num{1,11} \bar{V}\ped{rec}
	\]
	
	Per tant, només cal multiplicar per 1,11 el valor mesurat pel voltímetre (valor rectificat mitjà), per tal d'obtenir el valor desitjat (valor sinusoidal eficaç). De fet, el més usual és que la caràtula del voltímetre ja estigui ajustada amb aquest factor de 1,11, de manera que la lectura sigui directament la del valor eficaç.
\end{exemple}
...
\begin{exemple}[Càlcul de la potència en un sistema de 3 fils]\label{ex:calc-pot}
	\addcontentsxms{Càlcul de la potència en un sistema de 3 fils}
...
\begin{exemple}[Càrrega i descàrrega d'un circuit R-L]\label{ex:carrega-descarrega-RL}
	\addcontentsxms{Càrrega i descàrrega d'un circuit R-L}
    ...
    \begin{dingautolist}{'312}
        ...
        \item Tot seguit entrem en els camps \funsfbs{F1(X)}, \funsfbs{F2(X)} i \funsfbs{F3(X)} les equacions de $i_1(t)$, $i_2(t)$ i $i_3(t)$; en lloc dels valors \num{1,7293} i  \num{0,4558}, utilitzarem \funsfbs{F1(0.02)} i \funsfbs{F2(0.04)} respectivament, per tal que la calculadora els calculi automàticament. Com a variable utilitzarem \funsfbs{X} en lloc de \funsfbs{t}.
            ...
            Una altra manera de crear el camp \funsfbs{F4(X)}, és utilitzar la funció \funsfbs{IFTE(Cond,ST,SF)}; aquesta funció avalua l'expressió \funsfbs{Cond}, si el resultat   és cert, executa \funsfbs{ST} i si és fals  executa \funsfbs{SF}. Podem escriure per tant: \funsfbs{IFTE(X<0.02,F1(X),IFTE(X<0.04,F2(X),F3(X)))}.
...
\begin{exemple}[Curtcircuit en un circuit R-L]\label{ex:cc-RL}
	\addcontentsxms{Curtcircuit en un circuit R-L}
...
\begin{exemple}[Corrent de pic asimètric]\label{ex:corrent-pic}
	\addcontentsxms{Corrent de pic asimètric}
    ...
    El valor de curtcircuit eficaç simètric calculat a l'exemple \vref{ex:cc-RL} és: $I\ped{sim}= \qty{25,4}{kA}$.
    ...
    Aquest valor és més a prop del real, tal com es pot veure en la gràfica de l'exemple  \ref{ex:cc-RL}, que el valor de  \qty{71,9}{kA} obtingut en aquell exemple.
...
\begin{dingautolist}{'312}
   ...
   \item Cada element de fora de la diagonal de la matriu d'impedàncies és igual a la suma canviada de signe de les impedàncies en comú a les dues malles corresponents.
    ...
    \item Cada element del vector de fonts de tensió és igual a la suma de les fonts de tensió que formen part de cada malla; la contribució a la suma de cada font de tensió serà positiva o negativa, segons que la seva força electromotriu tingui el mateix sentit que el corrent de malla o que  tingui el sentit contrari.
         ...
         En el nostre exemple $\cmplx{E}_1$ i $\cmplx{I}\ped{M1}$ tenen el mateix sentit, i per tant tenim:   $\mcmplx{E}(1) = \cmplx{E}_1$, en canvi $\cmplx{E}_2$ i $\cmplx{I}\ped{M3}$ tenen   sentits contraris, i per tant   tenim:   $\mcmplx{E}(3) = -\cmplx{E}_2$, i com que la malla 2 no té cap font de tensió  tenim:   $\mcmplx{E}(2) = 0$.
\end{dingautolist}


\begin{exemple}[Aplicació del mètode de les malles]\label{ex:malles}
	\addcontentsxms{Aplicació del mètode de les malles}
