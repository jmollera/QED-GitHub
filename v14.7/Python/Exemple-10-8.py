...
s_100_fun = motor.s_match(T_load, U_1)
mot_100_fun = motor.run(s_100_fun, U_1)
mot_100_arr = motor.run(1, U_1)
mot_100_arrencada = motor.start_up(T_load, J, U_1)
print(f's,100% = {s_100_fun:.4f}')
print(f'n_m,100% = {motor.to_n_m(s=s_100_fun):.1f} r/min')
...
mot_80_arrencada = motor.start_up(T_load, J, U_1_80)
print(f's,80% = {s_80_fun:.4f}')
print(f'n_m,80% = {motor.to_n_m(s=s_80_fun):.1f} r/min')
...
for i, pu in enumerate(U1_pu):
    s[i] = motor.s_match(T_load, pu * U_1)
mot_sol = motor.run(s, U1_pu * U_1)
...
s_100_t_arr = mot_100_arrencada.s_start_up(t_arr)
mot_100_t_arr = motor.run(s_100_t_arr, U_1)
T_load_100_t_arr = T_load(s_100_t_arr)
s_80_t_arr = mot_80_arrencada.s_start_up(t_arr)
