...
llurs impedàncies o admitàncies sinó les potències que
absorbeixen, no podem emprar el mètode dels nusos descrit en el
capítol \ref{sec:ch-xarxes-elec} per  resoldre-la.
...
per fase en «$\pi$», format per una impedància longitudinal i dues
...
Finalment, la potència perduda en la  transmissió de la  línia $\Delta\cmplx{S}$, és definida per
...
tensió també en pot variar  l'argument; les tensions primària i
secundària tenen, per tant,   mòduls diferents i un desfasament entre llurs arguments.
...
Com es pot veure, tenim: $\mcmplx{Y}\ped{N}[1,2] \neq
\mcmplx{Y}\ped{N}[2,1]$; aquesta desigualtat ens indica que no existeix un esquema
equivalent en «$\pi$» del transformador, format únicament per
elements passius.
...
Finalment, la potència perduda en la  transmissió del transformador  $ \Delta\cmplx{S}$, és definida per
l'expressió:
...
(del nus 2 a l'1), i la potència perduda en la transmissió
...
El circuit equivalent d'aquest tipus de transformador també és el de la
 ...
 existeix un circuit equivalent en «$\pi$» format per una impedància longitudinal i dues admitàncies transversals, tal com es pot veure en la figura  \vref{pic:equiv_trafo_reg}.
...
de l'exterior de la xarxa, i el mòdul i l'argument de la
tensió.
...
Ara bé, si expressem els potencials $\cmplx{v}_i$ a partir de llurs mòduls $|\cmplx{v}_i|$
i arguments $\delta_i$, i les admitàncies $\cmplx{y}_{ik}$ a partir de llurs parts reals $g_{ik}$ i imaginàries $b_{ik}$, tenim:
\begin{align}
...
desconeguts (vegeu la taula \vref{taula:tipus_nusos}). Per tant, per
 resoldre el problema del flux de càrregues en un sistema
...
\begin{exemple}[\FluxCarrXarxa{} \hyperlink{exemple:FluxCarrXarxa}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:FluxCarrXarxa}
    ...
    Per tant, les potències subministrades a la xarxa per la xarxa extera del nus 1 i pel generador
    del nus 2, són:
    \begin{align*}
    \cmplx{s}\ped{X1} &= \cmplx{s}_1 = \complexqty{0,85680 + j 0,52169}{pu} \\[1ex]
...
\begin{exemple}[\ControlTensCond{} \hyperlink{exemple:ControlTensCond}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:ControlTensCond}
    ...
    \begin{enumerate}
       ...
       \item Es connecta el banc de condensadors del nus 2, per  mantenir el mòdul de
    ...
    Per tant, la potència subministrada a la xarxa per la xarxa externa del nus
    1, és:
    \[
    \cmplx{s}\ped{X1} = \cmplx{s}\ped{L1} + \cmplx{s}_{12} = \complexqty{1,2+j0,3}{pu} + \complexqty{0,82812 + j 0,59423}{pu} = \complexqty{2,02813 + j 0,89423}{pu}
    \]
    ...
    Per tant, les potències subministrades a la xarxa per la xarxa externa del
    nus 1 i per la bateria de condensadors del nus 2, són:
    \begin{align*}
     \cmplx{s}\ped{X1} &= \cmplx{s}\ped{L1} + \cmplx{s}_{12} = \complexqty{1,2+j0,3}{pu} + \complexqty{0,81695 - j 0,04520}{pu} =
    ...
    S'ha calculat el valor de $\cmplx{s}\ped{C2}$ per  comprovar
