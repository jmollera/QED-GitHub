...
tractament es fa  detalladament des del punt de vista de la norma CEI 60044 \textit{Instrument transformers}, no obstant això, es dedica també un
...
corrent, en canvi, es connecten amb llur primari intercalat en la
...
representar mitjançant fasors. L'error de fase $\epsilon_\phi$ es considera positiu quan la magnitud secundària avança a la primària.
\index{$\epsilon_\phi$}
...
minuts d'arc o en centiradians (crad).
...
Els errors de relació i de fase d'un transformador no són
...
de precisió», que és el valor de la potència aparent consumida per la càrrega ---expressada  en voltampere---
...
Els valors normalitzats per a transformadors connectats entre dues fases, són els indicats en la norma CEI 60038 \textit{IEC standard voltages}.\index{CEI!60038-00@60038}
...
multiplicar-se la tensió nominal primària, per  determinar la
tensió màxima que el Tt pot suportar durant un temps determinat,
sense sobrepassar ni l'escalfament admissible ni els límits dels errors
...
\index{transformadors de mesura i protecció (Tt)!identificació dels terminals}
 ...
 En el cas d'un Tt amb un  secundari de  múltiples preses, els terminals s'identifiquen amb les lletres  «a1», «a2», «a3», \ldots, «b» (o «n»).
...
Segons quina sigui la  funció, els Ti es classifiquen de forma
anàloga als Tt en:
\begin{itemize}
	\item \textbf{Transformadors de mesura.} Són els emprats per alimentar
...
són: \qtylist{10; 12,5; 15; 20; 25;30; 40; 50; 60;75}{A}, i llurs múltiples i submúltiples decimals.

Els valors preferits són: \qtylist{10; 15; 20; 30; 50;75}{A}, i llurs múltiples i submúltiples decimals.
...
Per a corrents de primari i secundari sinusoidals, l'error compost $\epsilon\ped{c}$ es defineix  en funció dels errors de relació $\epsilon\ped{r}$ i de fase  $\epsilon_\phi$, com:\index{$\epsilon\ped{c}$}
\begin{equation}
    \epsilon\ped{c} = \sqrt{\epsilon\ped{r}^2 +  \epsilon_\phi^2}
\end{equation}

En aquesta expressió, els errors $\epsilon\ped{c}$ i $\epsilon\ped{r}$  estan expressats en \si{\percent}, i l'error $\epsilon_\phi$ està expressat en crad.

En el cas general de corrents primaris $i\ped{p}(t)$ i secundaris $i\ped{s}(t)$ no sinusoidals, però periòdics amb període $T$, l'error compost $\epsilon\ped{c}$ es defineix com:
\begin{equation}
...
És possible especificar valors per sobre de \qty{30}{VA} segons les necessitats de cada cas.
...
$I\ped{cc}$ és el corrent de curtcircuit i $t$ n'és la durada
 expressada en segons, ha de complir-se: $I\ped{th}\geq
...
dels límits previstos i mantenint-se dins de la
 classe de precisió. El valor usual és: $I\ped{cth} = I\ped{np}$. Quan es requereix un valor més elevat, els valors preferits són:
...
secundari continuï reflectint el corrent de primari per a
 sobrecorrents elevats ---encara que sigui amb errors més grans--- perquè els relés de protecció connectats al transformador actuïn
...
secundari encara reflectirà amb suficient precisió el valor del corrent primari.
...
Valors típics per a la resistència del debanat secundari, obtinguts de la secció 1.6 de \cite{CAP}, són:
\begin{itemize}
...
\begin{dingautolist}{202}
    ...
    paraula  anglesa \textit{calculated} (calculada). El flux de dispersió d'aquests transformadors és negligible, i llur error es pot calcular.\index{C}\index{calculated@\textit{calculated}}

    La lletra «K» és equivalent a la «C», però es fa servir quan la tensió del colze de la corba d'excitació del transformador és igual o superior al \qty{70}{\percent}
    de la tensió nominal secundària.\index{K}

    La lletra «T» és la inicial de la   paraula  anglesa \textit{tested} (mesurada). El flux de dispersió d'aquests transformadors és apreciable, i llur error només es pot obtenir mitjançant un assaig.\index{T}\index{tested@\textit{tested}}
    ...
    que hi pot haver en el secundari, per no sobrepassar l'error compost que té
...
que ens demana el nou aparell que volem instaŀlar, per  no
equivocar-nos.
...
d'atenció, perquè a més de saber a  quins terminals de l'aparell hem
...
\begin{exemple}[\ConnexWatt{}]
    ...
    \item Ens fixem a continuació en els corrents del catàleg del  wattímetre. Si suposem
    ...
    circuit inicial del catàleg del wattímetre, veiem que els terminals per on surten
