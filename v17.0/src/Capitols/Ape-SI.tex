...
Internacional d'unitats (SI), el qual és publicat pel  \textit{Bureau
International des Poids et Mesures} (BIPM). S'ha utilitzat la 9a edició de març de 2019, revisada el desembre de  2022 amb l'addició dels prefixos  $10^{27}$,  $10^{30}$,  $10^{-27}$ i  $10^{-30}$, l'agost de 2024 amb la millora de la descripció dels angles i de les magnituds unitàries, i l'agost de 2025 amb l'addició dels prefixos binaris $2^{90}$ i  $2^{100}$, i l'actualització del valor del dalton. Podeu trobar més informació a: \href{https://www.bipm.org/en/home/}{https:/\!\!/www.bipm.org/en/home} i \href{https://www.bipm.org/en/publications/si-brochure/}{https:/\!\!/www.bipm.org/en/publications/si-brochure}. \index{BIPM}

El  \textit{National Institute of Standards and Technology} (NIST) també té informació referent al Sistema
Internacional d'unitats, a l'adreça: \href{https://www.nist.gov/pml/owm/metric-si}
{https:/\!\!/www.nist.gov/pml/owm/metric-si}.\index{NIST}
...
En la taula \vref{taula:SI-fonamentals} es poden veure les set unitats
...
\begin{list}{}
  ...
  hiperfins de l'estat fonamental de l'àtom de cesi-133. (1967).
   ...
   aquests dos conductors  una força igual a \num{2e-7} newtons per metre de longitud. (1948).
   ...
   d'una font que emet radiació monocromàtica de freqüència \qty{540e12}{hertzs}, i
   que té una intensitat radiant en aquesta direcció de $\frac{1}{683}$ watts per estereoradian. (1979).
\end{list}
...
\item La freqüència de la transició hiperfina de l'estat fonamental de l'àtom de cesi-133 no pertorbat, $\Delta\nu\ped{Cs}$, és igual a \qty{9 192 631 770}{Hz}.\index{$\Delta\nu\ped{Cs}$}
\item La velocitat de la llum en el buit, $c$, és igual a \qty{299792458}{m/s}.\index{velocitat!de la llum en el buit}\index{c@$c$}
\item La constant de Planck, $h$, és igual a \qty{6,62607015 e-34}{J.s}.\index{constants!de Planck}\index{h@$h$}
\item La càrrega elemental, $e$, és igual a \qty{1,602176634 e-19}{C}.\index{carrega@càrrega!elemental}\index{e@$e$}
\item La constant de Boltzmann, $k$, és igual a \qty{1,380649e-23}{J/K}.\index{constants!de Boltzmann}\index{k@$k$}
\item La constant d'Avogadro, $N\ped{A}$, és igual a \qty{6,02214076 e23}{mol^{-1}}.\index{constants!d'Avogadro}\index{NA@$N\ped{A}$}
\item L'eficàcia lluminosa d'una radiació monocromàtica de freqüència \qty{540e12}{Hz},  $K\ped{cd}$, és igual a \qty{683}{lm/W}.\index{eficàcia lluminosa}\index{Kcd@$K\ped{cd}$}
\end{itemize}
...
\begin{list}{}
   {\setlength{\labelwidth}{22mm} \setlength{\leftmargin}{22mm} \setlength{\labelsep}{2mm}}
   \item[\textbf{segon}] El segon, amb símbol s, es defineix a partir de la constant $\Delta\nu\ped{Cs}$ expressada en la unitat Hz, la qual és equivalent a $\unit{s^{-1}}$:
       \[
            \qty{1}{s} = \frac{\num{9192631770}}{\Delta\nu\ped{Cs}}
       \]
  ...
  hiperfins de l'estat fonamental de l'àtom de cesi-133 no pertorbat.
   ...
   \item[\textbf{metre}]  El metre, amb símbol m, es defineix a partir de la constant $c$ expressada en les unitats \unit{m.s^{-1}}, on el segon està definit en funció de la constant  $\Delta\nu\ped{Cs}$:
       \[
            \qty{1}{m} = \left(\frac{c}{\num{299792458}}\right)\unit{\,s} = \frac{\num{9192631770}}{\num{299792458}}\frac{c}{\Delta\nu\ped{Cs}}
       \]

    La conseqüència d'aquesta definició és que un metre és igual a la longitud de la trajectòria recorreguda per la llum
   en el buit, durant un temps de $\frac{1}{\num{299792458}}$ s.

   \item[\textbf{quilogram}] El quilogram, amb símbol kg, es defineix a partir de la constant $h$ expressada en les unitats \unit{J.s}, equivalents a \unit{kg.m^2.s^{-1}}, on el metre i el segon estan definits en funció de les constants $c$ i $\Delta\nu\ped{Cs}$:
       \[
            \qty{1}{kg} = \left(\frac{h}{\num{6,62607015 e-34}}\right)\unit{\,m^{-2}.s} = \frac{\num{299792458}^2}{\num{6,62607015 e-34}\times\num{9192631770}}\frac{h\Delta\nu\ped{Cs}}{c^2}
       \]
   ...
   \item[\textbf{ampere}] L'ampere, amb símbol A, es defineix a partir de la constant $e$ expressada en la unitat C,  equivalent a \unit{A.s}, on el segon està definit en funció de la constant  $\Delta\nu\ped{Cs}$:
       \[
            \qty{1}{A} = \left(\frac{e}{\num{1,602176634 e-19}}\right)\unit{\,s^{-1}} = \frac{1}{\num{9192631770}\times\num{1,602176634 e-19}} \Delta\nu\ped{Cs} e
       \]

    La conseqüència d'aquesta definició és que un ampere és igual al corrent elèctric corresponent al flux de $\frac{1}{\num{1,602176634e-19}}$   càrregues elementals per segon.

   \item[\textbf{kelvin}]  El kelvin, amb símbol K, es defineix a partir de la constant $k$ expressada en les unitats \unit{J.K^{-1}},  equivalents a \unit{kg.m^2.s^{-2}.K^{-1}},  on el quilogram, el metre i el segon estan definits en funció de les constants $h$, $c$ i $\Delta\nu\ped{Cs}$:
       \[
            \qty{1}{K} = \left(\frac{\num{1,380649e-23}}{k}\right)\unit{\,kg.m^2.s^{-2}} = \frac{\num{1,380649e-23}}{\num{6,62607015 e-34}\times\num{9192631770}} \frac{\Delta\nu\ped{Cs} h}{k}
       \]
   ...
   \item[\textbf{candela}] La candela, amb símbol cd, es defineix a partir de la constant $K\ped{cd}$ expressada en les unitats \unit{lm.W^{-1}},  equivalents a \unit{cd.sr.kg^{-1}.m^{-2}.s^3},  on el quilogram, el metre i el segon estan definits en funció de les constants $h$, $c$ i $\Delta\nu\ped{Cs}$:
       \[
            \qty{1}{cd} = \left(\frac{K\ped{cd}}{683}\right)\unit{\,kg.m^2.s^{-3}.sr^{-1}} = \frac{1}{\num{6,62607015 e-34}\times\num{9192631770}^2\times 683} \Delta\nu\ped{Cs}^2 h K\ped{cd}
       \]
...
Internacional d'unitats, per  formar-ne els múltiples i
submúltiples.
...
\label{sec:unitats-derv-SI}
\index{sistema internacional d'unitats!unitats derivades amb noms i símbols propis}

De forma convenient, s'ha donat noms i símbols propis a vint-i-dues unitats derivades de les fonamentals; en la taula \vref{taula:SI-derivades} es mostren aquestes unitats derivades de l'SI.

\begin{ThreePartTable}
\begin{TableNotes}
    \item[\color{blue}(a)] {\footnotesize La variant ortogràfica «radiant» també és correcta. Un radian és   l'angle subtendit al centre d'un cercle per un arc de longitud igual al radi. Això suggereix que rad = m/m, però aquesta representació no és  intrínseca i podria dur a confusió perquè l'angle no és  una quantitat de la mateixa naturalesa que altres relacions de longituds. Una altra definició possible seria que  l'angle recte es igual a $\pi/2$ rad. El radian és també la unitat de l'angle de fase. Per als fenòmens periòdics, l'angle de fase augmenta en $2 \pi$ rad a cada període.}
    \item[\color{blue}(b)] {\footnotesize La variant ortogràfica «estereoradiant» també és correcta. Un estereoradian és l'angle sòlid subtendit al centre d'una esfera per una àrea de la superfície que és igual al radi al quadrat. Això suggereix que sr = \unit{m^2/m^2}, però aquesta representació no és intrínseca i podria dur a confusió perquè l'angle sòlid no és una quantitat de la mateixa naturalesa que altres relacions de superfícies. Una altra definició possible seria que una esfera sencera subtendeix, des del seu centre, un angle sòlid de $4 \pi$  sr.}
    \item[\color{blue}(c)] {\footnotesize L'hertz només s'ha d'utilitzar per a fenòmens periòdics i el becquerel només per a  processos aleatoris vinculats a la mesura de l'activitat d'un radionúclid.}
     \item[\color{blue}(d)] {\footnotesize En fotometria, generalment mantenim el nom i el símbol de l'estereoradian, sr, en l'expressió d'unitats.}
    \item[\color{blue}(e)] {\footnotesize Acrònim de l'expressió anglesa \textit{kinetic energy released  per unit mass}, o també \textit{kinetic energy released in matter}.}
\end{TableNotes}
\begin{longtable}[h]{llclc}
   ...
   \insertTableNotes
   \endlastfoot
   angle pla & radian\tnote{\color{blue}(a)} & rad   & --- & 1\\
   angle sòlid & estereoradian\tnote{\color{blue}(b)} & sr & ---  & 1 \\
   freqüència & hertz\tnote{\color{blue}(c)} & Hz & \unit{s^{-1}} & --- \\
   ...
   temperatura Celsius & grau Celsius &  \unit{\degreeCelsius} & K & --- \\
   flux lluminós & lumen & lm  & \unit{cd.sr}\tnote{\color{blue}(d)} & \unit{cd.sr}\\
   iŀluminació & lux & lx & \unit{cd.sr.m^{-2}} & \unit{lm/m^2} \\
   activitat  d'un radionúclid & becquerel\tnote{\color{blue}(c)} & Bq& \unit{s^{-1}} & --- \\
   dosi absorbida, kerma\tnote{\color{blue}(e)} & gray & Gy  & \unit{m^2.s^{-2}}& \unit{J/kg}\\
...
\index{carrega@càrrega!elèctrica} \index{potencial
...
Les set unitats fonamentals (vegeu la taula \vref{taula:SI-fonamentals}) i les vint-i-dues unitats derivades amb noms i símbols propis (vegeu la taula \vref{taula:SI-derivades}) poden combinar-se entre si per crear noves unitats derivades.
...
\begin{longtable}[h]{lll}
   ...
   exposició (raigs $x$ i $\gamma$) & \unit{C/kg} & \unit{A.s.kg^{-1}} \\
...
\begin{TableNotes}
    ...
    \item[\color{blue}(e)] {\footnotesize El «dalton» i la «unitat de massa atòmica unificada», amb símbol «u»,  són dos noms alternatius d'una mateixa unitat. El seu valor és igual a $\frac{1}{12}$ de la massa de l'àtom de carboni-12 lliure, en repòs i en el seu estat fonamental (vegeu el valor de $m\ped{u}$ a la taula \vref{taula:Const-Fis}).}
    \item[\color{blue}(f)] {\footnotesize Un «electró-volt» és l'energia cinètica que adquireix un electró després de creuar una diferència de potencial d'un volt en el buit. El seu valor és exacte (vegeu el valor de eV la taula \vref{taula:Const-Fis}).}
...
\begin{longtable}[h]{llcl}
   ...
   angle pla o de fase & grau &  \unit{\degree} &   $\ang{1} = \frac{\pi}{180}\unit{\,rad}$ \\[2mm]
   angle pla o de fase & minut & \unit{\arcminute} & $\ang{;1;} = \big(\frac{1}{60}\big)\raisebox{1.5mm}{\unit{\degree}} = \frac{\pi}{10800}\unit{\,rad}$ \\[2mm]
   angle pla  o de fase & segon & \unit{\arcsecond} & $\ang{;;1} = \big(\frac{1}{60}\big)' = \frac{\pi}{648000}\unit{\,rad}$ \\[2mm]
   ...
   massa & dalton\tnote{\color{blue}(e)} & Da & $\qty{1}{Da} = \qty{1,660 539 068 92(52)e-27}{kg}$\\
...
\index{unitats!astronomica@astronòmica}\index{au}\index{temps}\index{angle!pla}\index{area@àrea}
\index{volum}\index{massa}\index{longitud}
\index{electró-volt}\index{unitats!de massa atomica unificada@de massa atòmica unificada}
...
Es dona, no obstant això, en la taula \vref{taula:SI-altres-NIST} quatre unitats fora de l'SI acceptades addicionalment pel NIST en el document \textit{Special Publication 811: Guide for the Use of the International System of Units (SI)}, ja que s'han utilitzat tradicionalment als Estats Units d'Amèrica; aquest organisme desaconsella però continuar usant-les, i recomana l'ús de les unitats equivalents de l'SI.\index{NIST}
...
\begin{longtable}[h]{llcl}
   ...
   \endlastfoot
    ...
    exposició (raigs $x$ i $\gamma$) & roentgen & \unit{R} & \qty{1}{R} = \qty{2,58e-4}{C/kg} \\
\bottomrule[1pt]
\end{longtable}
\end{ThreePartTable}
...
\subsection{Unitats informàtiques i prefixos de potències binàries}\label{sec:bit-octet-byte}
...
La norma IEEE 1541 \textit{Standard for Prefixes for Binary Multiples}, defineix les tres mateixes unitats i fa la mateixa distinció entre octet i byte. L'única diferència és que assigna al bit el símbol «b» en lloc del símbol «bit».\index{IEEE!1541}

En la taula \vref{taula:prefix-inform} es mostren els prefixos de potències binàries. Els prefixos «robi» i «quebi» s'han inclòs en la revisió de l'any 2025 de la norma CEI 80000-13.
\begin{longtable}[h]{lccll}
   ...
   \endfoot
   \endlastfoot
   quebi & Qi   & $2^{100}$ & \num{1267650600228229401496703205376} &  $\quad\num{1,268e30}$ \\
   robi & Ri   & $2^{90\phantom{0}}$ & \num{1237940039285380274899124224} &  $\quad\num{1,238e27}$ \\
   yobi & Yi   & $2^{80\phantom{0}}$ & \num{1208925819614629174706176} &  $\quad\num{1,209e24}$ \\
   zebi & Zi   & $2^{70\phantom{0}}$ & \num{1180591620717411303424}&  $\quad\num{1,181e21}$ \\
   exbi & Ei   & $2^{60\phantom{0}}$ & \num{1152921504606846976}&  $\quad\num{1,153e18}$ \\
   pebi & Pi   & $2^{50\phantom{0}}$ & \num{1125899906842624}&  $\quad\num{1,126e15}$ \\
   tebi & Ti   & $2^{40\phantom{0}}$ & \num{1099511627776}&  $\quad\num{1,100e12}$ \\
   gibi & Gi   & $2^{30\phantom{0}}$ & \num{1073741824}&  $\quad\num{1,074e9}$  \\
   mebi & Mi   & $2^{20\phantom{0}}$ & \num{1048576} &  $\quad\num{1,049e6}$ \\
   kibi & Ki   & $2^{10\phantom{0}}$ & \num{1024} & $\quad\num{1,024e3}$  \\
   \bottomrule[1pt]
\end{longtable}
\index{kibi} \index{Ki} \index{mebi} \index{Mi} \index{gibi}  \index{Gi} \index{tebi} \index{Ti} \index{pebi} \index{Pi} \index{exbi} \index{Ei} \index{zebi} \index{Zi} \index{yobi} \index{Yi} \index{robi} \index{Ri} \index{quebi} \index{Qi} 
...
\begin{ThreePartTable}
	\begin{TableNotes}
		\item[\color{blue}(a)] {\footnotesize En alguns llibres i publicacions tècniques es poden veure els símbols no estàndard «VAR»,  «Var» i «VAr».}
	\end{TableNotes}
\begin{longtable}[h]{llcl}
...
\begin{longtable}[h]{llcl}
   ...
   \endfoot
   \endlastfoot
   angle pla & revolució &  \unit{r} & $\qty{1}{r} = 2\pi\unit{\,rad}$  = 1 (cicle)\\
   velocitat angular & revolució per segon &  \unit{r/s}& $\qty{1}{r/s} = 2\pi\unit{\,rad/s} = \qty{1}{Hz}$  \\
   velocitat angular & revolució per minut &  \unit{r/min}& $\qty{1}{r/min} = \frac{2\pi}{60}\unit{\,rad/s} = \frac{1}{60}\unit{\,Hz}$  \\
   \bottomrule[1pt]
\end{longtable}
...
\item Un estil comú d'escriptura de textos tècnics i científics representa amb lletra inclinada els noms de les variables.
Els noms i els símbols de les unitats, això no obstant, s'escriuen sempre amb lletra recta.
També s'escriuen amb lletra recta els valors numèrics.

\textcolor{Green}\faCheckSquare{} \textit{P} = \textit{U} \textit{I} = \qty{220}{V} $\times$ \qty{5}{A} = \qty{1100}{W}

\textcolor{Red}\faTimesCircle{} \textit{P} = \textit{U} \textit{I} = 220 \textit{V} $\times$ 5 \textit{A} = 1100 \textit{W}

\textcolor{Green}\faCheckSquare{} \textit{l} = 1 metre

\textcolor{Red}\faTimesCircle{} \textit{l} = 1 \textit{metre}
...
\item Els símbols de les unitats no canvien de forma en el plural. \footnote{Els noms de les unitats,  en canvi, són substantius i com a tals segueixen les normes ortogràfiques del català a l'hora de formar el plural. Així escriurem en plural: farads, candeles, metres, segons, hertzs, kèlvins, graus Celsius, etc. Vegeu l'apartat 7.3 de la \textit{Gramàtica de la llengua catalana} de l'IEC (\href{https://giec.iec.cat/index-general}{https:/\!\!/giec.iec.cat/index-general}) i el \textit{Diccionari essencial de la llengua catalana} de l'IEC (\href{https://deiec.iec.cat/}{https:/\!\!/deiec.iec.cat}).
} No han
...
del nom de la unitat amb el símbol «\unit{\degreeCelsius}» és «grau Celsius», perquè la unitat  «grau» comença per la lletra g  minúscula i el qualificatiu «Celsius» comença per la lletra C  majúscula, atès que és un nom propi.

\textcolor{Green}\faCheckSquare{} 10 newtons

\textcolor{Red}\faTimesCircle{} 10 Newtons

\textcolor{Green}\faCheckSquare{}  100 watts

\textcolor{Red}\faTimesCircle{} 100 Watts

\textcolor{Green}\faCheckSquare{}  24 volts

\textcolor{Red}\faTimesCircle{} 24 Volts

\textcolor{Green}\faCheckSquare{}  20 graus Celsius

\textcolor{Red}\faTimesCircle{} 20 graus celsius
...
\textcolor{Green}\faCheckSquare{} 4 radians per segon

\textcolor{Red}\faTimesCircle{} 4 radians/s

\textcolor{Green}\faCheckSquare{} \qty{100}{km/h}

\textcolor{Red}\faTimesCircle{} 100 km/hora

\textcolor{Green}\faCheckSquare{} 100 quilòmetres per hora

\textcolor{Red}\faTimesCircle{} 100 quilòmetres/h
...
és millor evitar-les, per exemple: \qty{1}{N.m} (1 newton metre) i
\qty{1}{m.N} (1 metre newton) són expressions equivalents, però
aquesta darrera forma d'escriptura pot ser confosa amb \qty{1}{mN} (1~miŀlinewton).

\textcolor{Green}\faCheckSquare{}  \qty{1}{N.m}

\textcolor{Blue}\faExclamationTriangle{} \qty{1}{m.N}
...
\textcolor{Green}\faCheckSquare{}  \qty{12}{W/mm^2}

\textcolor{Red}\faTimesCircle{}  \qty{12}{W/(mm)^2}
...
\textcolor{Green}\faCheckSquare{} \qty{1}{MV/m}

\textcolor{Blue}\faExclamationTriangle{}  \qty{1}{kV/mm}


\item De forma anàloga, el mateix és aplicable als símbols d'unitats
derivades formades pel producte d'altres unitats.

\textcolor{Green}\faCheckSquare{} \qty{25}{N.m}

\textcolor{Blue}\faExclamationTriangle{}  \qty{25}{kN.mm}
...
\textcolor{Green}\faCheckSquare{}  980 hectopascals

\textcolor{Red}\faTimesCircle{} 980 hecto pascals
...
\textcolor{Green}\faCheckSquare{} 1200 megawatts

\textcolor{Red}\faTimesCircle{} 1200 Megawatts

\textcolor{Green}\faCheckSquare{} 12 gigahertzs

\textcolor{Red}\faTimesCircle{} 12 Gigahertzs
...
\textcolor{Red}\faTimesCircle{}  \qty{1}{m/s} és 1 metre dividit per segon
 ...
 \textcolor{Green}\faCheckSquare{} \qty{20}{\ohm.m} són 20 ohms metre

\textcolor{Red}\faTimesCircle{}   \qty{20}{\ohm.m} són 20 ohms  per metre

\textcolor{Red}\faTimesCircle{}   \qty{20}{\ohm.m} són 20 ohms multiplicats per metre


\item El valor d'una quantitat ha d'expressar-se  utilitzant únicament una unitat.
...
\item Són una excepció a la regla anterior les magnituds de temps i d'angle pla de la taula \vref{taula:SI-altres-acceptades}; tot i això, pel que fa a l'angle pla és preferible seguir la regla general excepte en disciplines on s'utilitzen angles molt petits, com ara la navegació, la cartografia o l'astronomia.

\textcolor{Green}\faCheckSquare{} 2 d 13 h 12 min

\textcolor{Red}\faTimesCircle{}   \qty{2,55}{d}

\textcolor{Green}\faCheckSquare{} \ang{22,04}

\textcolor{Blue}\faExclamationTriangle{} \ang{22;2;24}
...
\textcolor{Green}\faCheckSquare{} $U\ped{DC} =\qty{24}{V}$

\textcolor{Red}\faTimesCircle{} $U = \qty{24}{VDC}$
...
\item Quan un valor té moltes xifres, les xifres poden dividir-se en grups de tres mitjançant un espai curt per millorar-ne la llegibilitat. No s'han d'utilitzar punts o comes per separar aquests grups de tres xifres.
...
El NIST, en el document \textit{Special Publication 811:
Guide for the Use of the International System of Units (SI)},  fa a més les recomanacions següents:

\begin{itemize}

\item Quan s'indiquen valors de magnituds amb llurs precisions,
...
La pàgina web en  qüestió, \href{https://www.nist.gov/pml/pubs/sp811/appenb.cfm}{https:/\!\!/www.nist.gov/pml/pubs/sp811/appenb.cfm}, correspon a l'apèndix B del document \textit{Special Publication 811: Guide for the Use of the International System of Units (SI)}.
