...
Es descriu en aquesta secció l'ús dels polinomis de Lagrange\footnote{Joseph-Louis Lagrange, matemàtic francès d'origen italià: \href{https://en.wikipedia.org/wiki/Joseph-Louis_Lagrange}{https:/\!\!/en.wikipedia.org/wiki/Joseph-Louis\_Lagrange}.}  en la interpolació de dades.
...
\begin{exemple}[\InterpDuesDim{} \hyperlink{exemple:InterpDuesDim}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:InterpDuesDim}
    ...
    Com que tenim dades suficients, podríem utilitzar una interpolació cúbica, no obstant això, farem servir una interpolació lineal, per simplificar els càlculs, utilitzant les dades corresponents  a les concentracions de glicol etilè del  \qtylist{50;60}{\percent}, i a les temperatures de  \qtylist{70;80}{\degree F}.
...
Es descriuen en aquesta secció diversos mètodes d'integració numèrica de funcions, ja sigui coneixent només una sèrie de punts de la funció: $(x_1, y_1), (x_2, y_2), \dots ,(x_n, y_n)$, o ja sigui coneixent-ne l'expressió explícita: $y=f(x)$. La integral $\mathscr{I}$ de la funció $f(x)$ entre $x_1$ i $x_n$, ens donarà l'àrea amb signe que existeix entre la funció i l'eix d'abscisses.\footnote{Les parts de la funció per sobre de l'eix d'abscisses contribueixen a l'àrea amb signe positiu, i les parts per sota hi contribueixen amb signe negatiu. Per tant, l'àrea total pot ser positiva, negativa o nuŀla}

 \begin{equation}
    \mathscr{I} = \int_{x_1}^{x_n} f(x) \diff x
 \end{equation}
...
La integral $\mathscr{I}$ entre el punt inicial $(x_1, y_1)$ i el punt final $(x_n, y_n)$, es pot aproximar amb l'equació:

 \begin{equation}\label{eq:trap-uneven}
    \mathscr{I} \approx \sum_{i=1}^{n-1} \frac{y_{i+1} + y_i}{2} (x_{i+1}-x_i) =
     \frac{1}{2} \left( y_1(x_2-x_1) + y_n(x_n-x_{n-1}) +
       \sum_{i=2}^{n-1} y_i(x_{i+1}-x_{i-1})\right)
 \end{equation}
...
En el cas que les $N$ divisions tinguin la mateixa amplada, els  $n$ punts estaran separats uniformement una distància $h \equiv x_2-x_1 = x_3-x_2 = \dots = x_n-x_{n-1}$, i la regla dels trapezis esdevé:
 ...
 \begin{equation}\label{eq:trap}
    \mathscr{I} \approx \frac{h}{2} \left( y_1 + y_n + 2 \sum_{i=2}^{n-1} y_i \right)
 \end{equation}
...
La integral $\mathscr{I}$ entre el punt inicial $(x_1, y_1)$ i el punt final $(x_n, y_n)$, es pot aproximar amb l'equació:
 ...
 \begin{equation}\label{eq:simp_1_3}
   \mathscr{I} \approx  \frac{h}{3} \sum_{i=1,3,5\dots}^{n-2} \hspace{-0.5em} \big( y_i + 4 y_{i+1} + y_{i+2} \big) =
...
La integral $\mathscr{I}$ entre el punt inicial $(x_1, y_1)$ i el punt final $(x_n, y_n)$, es pot aproximar amb l'equació:
 ...
 \begin{equation}\label{eq:simp_3_8}
   \mathscr{I} \approx  \frac{3h}{8} \sum_{i=1,4,7\dots}^{n-3} \hspace{-0.5em} \big( y_i + 3 y_{i+1} + 3 y_{i+2} + y_{i+3}\big) =
...
\begin{exemple}[\IntegracioNum{} \hyperlink{exemple:IntegracioNum}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:IntegracioNum}
    ...
    Calculem primer el valor exacte d'aquesta integral, per  poder-lo comparar amb els que obtindrem  numèricament:
...
\begin{exemple}[\SolFunNoLin{} \hyperlink{exemple:SolFunNoLin}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:SolFunNoLin}
    ...
    Utilitzant la funció $i(t)$ obtinguda en exemple \vref{ex:CurtcircuitRL}, es tracta de calcular el punt proper a $t = \qty{20}{ms}$, on es compleix $i(t)=0$. Per adoptar la nomenclatura d'aquesta secció, substituïm $i(t)$ i $t$, per $f(x)$ i $x$ respectivament; l'equació que volem resoldre és doncs:
    \[
...
\section{Programes de càlcul numèric}\label{sec:calcul-numeric}

En la secció \vref{sec:HP_MATH} es donen  una sèrie de programes escrits per a la calculadora \textsf{HP Prime},
\index{HP Prime@\textsf{HP Prime}!exemples} que fan servir les equacions que han aparegut en aquest capítol. En concret tenim:

\begin{itemize}
   \item \texttt{Polynomial\_Interpolation} (llistat \vref{lst:PolyInterp}. Realitza una interpolació de dades utilitzant els polinomis de Lagrange.
    \index{HP Prime@\textsf{HP Prime}!funcions!PolynomialInterpolation@\texttt{Polynomial\_Interpolation}}
   \item \texttt{Polynomial\_Interpolation\_2D} (llistat \vref{lst:PolyInterp2D}. Realitza una interpolació de dades bidimensionals utilitzant els polinomis de Lagrange.
    \index{HP Prime@\textsf{HP Prime}!funcions!PolynomialInterpolation2D@\texttt{Polynomial\_Interpolation\_2D}}
    \item \texttt{Trapezoidal\_Rule} (llistat \vref{lst:TrapRule}. Realitza la integració d'una funció definida per una sèrie de punts, utilitzant la regla dels trapezis.
    \index{HP Prime@\textsf{HP Prime}!funcions!TrapezoidalRule@\texttt{Trapezoidal\_Rule}}
    \item \texttt{Simpson\_1\_3\_Rule} (llistat \vref{lst:Simp13Rule}. Realitza la integració d'una funció definida per una sèrie de punts, utilitzant la regla de Simpson 1/3.
    \item \texttt{Simpson\_1\_3\_Rule} (llistat \vref{lst:Simp13Rule}. Realitza la integració d'una funció definida per una sèrie de punts, utilitzant la regla de Simpson 1/3.
    \index{HP Prime@\textsf{HP Prime}!funcions!Simpson13Rule@\texttt{Simpson\_1\_3\_Rule}}
    \item \texttt{Simpson\_3\_8\_Rule} (llistat \vref{lst:Simp38Rule}. Realitza la integració d'una funció definida per una sèrie de punts, utilitzant la regla de Simpson 3/8.
    \index{HP Prime@\textsf{HP Prime}!funcions!Simpson38Rule@\texttt{Simpson\_3\_8\_Rule}}
\end{itemize}
