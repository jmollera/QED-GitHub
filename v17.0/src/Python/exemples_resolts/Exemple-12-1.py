...
f_sym = [0.25 - 0.5 - 1.03*(1.05*(-2.8*COS(-δ2) - 9.6*SIN(-δ2)) + 1.03*7.8 + v3*(-5*COS(δ3-δ2)-15*SIN(δ3-δ2))),
...
Jf_sym = sympy.Matrix(f_sym).jacobian(sympy.Matrix([δ2, v3, δ3]))

f_num = sympy.lambdify([(δ2, v3, δ3)], f_sym, 'numpy')
Jf_num = sympy.lambdify([(δ2, v3, δ3)], Jf_sym, 'numpy')
...
b3 = pp.create_bus(net, vn_kv=1, name='Nus 3')

pp.create_ext_grid(net, b1, vm_pu=1.05, va_degree=0, name='Xarxa 1')
pp.create_gen(net, b2, p_mw=0.25, vm_pu=1.03, name='Gen 2')
