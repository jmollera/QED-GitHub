...
print(f'n_m,n = {motor.convert(s_n, Speed.slip_to_n) :.1f} r/min,  '
      f'ω_m,n = {motor.convert(s_n, Speed.slip_to_ω) :.3f} rad/s\n')
...
Z_arr = mot_arr.Z
k = 1.02 + 0.98 * np.exp(-3 * Z_arr.real / Z_arr.imag)
I_1_arr_pic_asim = k * np.sqrt(2) * I_1_arr
...
mot_n_m = motor.run(motor.convert(n_m, Speed.n_to_slip), U_1)
...
ax.set_yticks(yticks, [f'{tick:.1f}'.replace('.', ',') for tick in yticks])
ax.grid()
