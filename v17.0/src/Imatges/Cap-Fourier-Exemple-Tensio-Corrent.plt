...
set xtics 0.00000,1.0,16.000 norangelimit textcolor rgb "black" 
set ytics border out scale 1,0.5 nomirror norotate  autojustify
set ytics -25.0000,5,25.0000 norangelimit textcolor rgb "black" 
...
set y2tics -1.2500,0.2500,1.2500 norangelimit textcolor rgb "black"
...
set xrange [ 0.0000 : 16.0000 ] noreverse nowriteback
set x2range [ 0.0000 : 16.0000 ] noreverse nowriteback
...
set y2range [ -1.2500 : 1.2500 ] noreverse nowriteback
set zlabel "" 
...
set output "Cap-Fourier-Exemple-Tensio-Corrent.tex"
set object rectangle from 0,-25 to 16,25 fillstyle solid fillcolor rgb "#FFFFF0" behind
z(n)=10+10*n*pi*{0,1}
plot 80.0/pi*(sum[k=1:30] sin((2*k-1)*0.2*pi*x)/(2*k-1)) axes x1y1 linecolor rgb "red" linewidth 6.000 title '$u(t)$', 80.0/pi*(sum[k=1:30] sin((2*k-1)*0.2*pi*x-arg(z(2*k-1)))/(2*k-1)/abs(z(2*k-1))) axes x1y2 linecolor rgb "sea-green"  linewidth 6.000 title '$i(t)$'
#    EOF
