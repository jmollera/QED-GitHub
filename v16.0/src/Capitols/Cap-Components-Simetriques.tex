...
igual a la unitat i un argument igual a \ang{120}: \index{a@$a$}\begin{equation}
   a \equiv \complexnum{1:120} = e^{j\frac{2\pi}{3}} = -\frac{1}{2} + j \frac{\sqrt{3}}{2}
\end{equation}
...
\begin{subequations}
\begin{align}
    a^2 &= \complexnum{1:240} = e^{-j\frac{2\pi}{3}} = -\frac{1}{2} - j \frac{\sqrt{3}}{2} \\
...
dels fasors dels  sistemes directe, invers i homopolar de la mateixa figura:
\begin{subequations}
\begin{align}
...
A partir del sistema d'equacions anterior, podem trobar els fasors dels sistemes directe, invers i homopolar en funció dels fasors del sistema
asimètric:
\begin{subequations}
\begin{align}
...
\begin{exemple}[\ImpedEquil{} \hyperlink{exemple:ImpedEquil}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:ImpedEquil}
    ...
    \vref{sec:lleis-trig-triang}): \index{llei!dels cosinus}
    \begin{align*}
    ...
    resistències, aplicant les lleis de Kirchhoff; com que les tres resistències són idèntiques, llurs  components    directa, inversa i homopolar $R_1$,  $R_2$ i $R_0$ també ho són i tenen el mateix valor de $\qty{10}{\ohm}$.
    \begin{align*}
    \cmplx{I}\ped{A,1} &=
