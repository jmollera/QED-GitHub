...
Un cop resolt el circuit, es multipliquen les magnituds obtingudes en tant per u per
llurs valors base, per obtenir les magnituds reals:
\begin{equation}
...
\begin{exemple}[\MetodeCalculPU{}]\label{ex:MetodeCalculPU}
    ...
    \end{align*}
     ...
     $1\!:\!1$, en coincidir les tensions base amb les tensions nominals.
...
\subsection{Valors base per a branques amb acoblament magnètic que treballen a diferent tensió}\index{pu!acoblament magnètic}
...
Per calcular la impedància base $Z_{\text{B}_\text{M}}$ i convertir $\cmplx{Z}\ped{M}$ en un valor en tant per u, no podem utilitzar les equacions \eqref{eq:bases_pu}, ja que cadascuna de les dues branques de l'acoblament magnètic està a una tensió diferent; en aquest cas cal utilitzar l'equació que podem trobar a la secció 1.7.5 de \cite{TLE}:
\begin{equation}
...
\subsection{Valors base per a branques amb acoblament capacitiu que treballen a diferent tensió}\index{pu!acoblament capacitiu}
...
Per  calcular l'admitància base $Y_{\text{B}_\text{M}}$ i convertir $\cmplx{Y}\ped{M}$ en un valor en tant per u, no podem utilitzar les equacions \eqref{eq:bases_pu}, ja que cadascuna de les dues branques de l'acoblament capacitiu està a una tensió diferent; en aquest cas cal utilitzar l'equació que podem trobar a la secció 1.7.5 de \cite{TLE}:
\begin{equation}
...
Cal puntualitzar que la transformació $\text{Y}\rightarrow\Delta$ no és vàlida si el  neutre de l'estrella  està connectat a terra i per aquesta connexió hi circula corrent.
...
Finalment, separem l'equació \eqref{eq:EZS_6} en dues, una per a la part real i una altra per a la part imaginària.
Si tenim en compte que $|\cmplx{U}'|^2 = \Re^2\cmplx{U}' + \Im^2\cmplx{U}'$, obtenim:
...
\begin{exemple}[\ResCircPotAbs{} \hyperlink{exemple:ResCircPotAbs}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:ResCircPotAbs}
    ...
    \item A continuació, suposant  que la calculadora està en el mode \textsfs{RPN}, entrem els valors de $\cmplx{E}$, $\cmplx{Z}$ i $\cmplx{S}$: \textsfs{(0.4,0.3)} \includegraphics{HPPrime-Enter.pdf} \textsfs{(0,0.1)} \includegraphics{HPPrime-Enter.pdf} \textsfs{(0.6,0.45)} \includegraphics{HPPrime-Enter.pdf}, i  escrivim després el nom del programa: \textsfs{EZS\_U()}.
    ...
    \item Premem a continuació la tecla \includegraphics{HPPrime-Enter.pdf} i la calculadora ens dona el valor de la tensió $\cmplx{U}$.
     ...
     \item   Si la calculadora estigués en el mode \textsfs{Algebraic} o en el mode  \textsfs{Textbook}, escriuríem directament: \textsfs{EZS\_U(0.4+0.3*}\textit{\textbf{i}}, \textsfs{0.1*}\textit{\textbf{i}}, \textsfs{0.6+0.45*}\textit{\textbf{i}}\textsfs{)}, i després de prémer la tecla \includegraphics{HPPrime-Enter.pdf} la calculadora ens donaria el valor de la tensió $\cmplx{U}$.
...
Per simplificar el problema, suposarem que tant la impedància
...
fusibles, on llurs corbes característiques corrent-temps estan representades en
...
\begin{exemple}[\ValorsEscalaLog{}]
    ...
    \[
        x = 10^{2+\frac{\qty{11}{mm}}{\qty{56}{mm}}}= \num{157,19}
    \]
...
Aquest tipus d'equació es compleix, per exemple, en el cas dels cables elèctrics, entre el corrent de curtcircuit i el temps que el cable pot aguantar aquest corrent sense malmetre's. Vegeu la secció \vref{ces:cables_Icc_termica}.
