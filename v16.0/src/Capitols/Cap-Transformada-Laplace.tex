...
\begin{equation}
    ...
    \varmathscr{L}^{-1}\{F(s)\} = f(t) \equiv \frac{1}{2 \pi j }
...
\subsection{Funció graó unitari i funció impuls}\index{funcions!graó
unitari} \index{funcions!impuls}\index{funcions!de
Heaviside}\index{funcions!delta de Dirac}\index{Heaviside, funció
...
$\varepsilon_\tau(t) \equiv \varepsilon(t-\tau)$ en l'instant de temps
$t=\tau$ es defineix com:
\begin{equation}
    \varepsilon_\tau(t) \equiv \varepsilon(t-\tau) = \begin{cases} 1, & t \geq \tau \\ 0, & t < \tau \end{cases}
\end{equation}

La funció impuls o funció delta de Dirac\footnote{Paul Adrien Maurice Dirac, matemàtic i físic britànic: \href{https://en.wikipedia.org/wiki/Paul_Dirac}{https:/\!\!/en.wikipedia.org/wiki/Paul\_Dirac}.} $\delta_\tau(t) \equiv
...
\begin{align}
    ...
    \int_0^\infty \delta_\tau(t) \diff t &= 1
\end{align}
\end{subequations}
...
\begin{align}
   ...
   \int_0^\infty f(t) \delta_\tau (t) \diff t &= f(\tau) \\[0.5ex]
...
\begin{alignat}{2}
    \varmathscr{L} \{ f(t - \tau)
    \varepsilon_\tau(t) \} &= e^{-s \tau} F(s), &\qquad \tau \in \mathsfb{R}_{\geq 0} \\
    \varmathscr{L}^{-1} \{ e^{-s \tau} F(s) \} &=
    f(t-\tau) \varepsilon_\tau(t), &\qquad \tau \in \mathsfb{R}_{\geq 0}
\end{alignat}
\end{subequations}
...
\subsection{Producte de convolució}\label{sec:prod-convolucio}\index{producte de convolució}
...
velocitat angular, amb: $\omega=2\pi f=2\pi/T$.
...
\begin{longtable}{r<{\hspace{3em}}l}
   ...
   \endfoot
   \endlastfoot
   $\varepsilon_\tau(t), \quad \tau \in\mathsfb{R}_{\geq 0}$  & $\dfrac{e^{-\tau s}}{s}$\\[2.2ex]
   $\delta_\tau(t), \quad\tau \in \mathsfb{R}_{\geq 0}$ & $e^{-\tau s}$\\[2.2ex]
   1 & $\dfrac{1}{s}$\\[2.2ex]
   $t$ &   $\dfrac{1}{s^2}$\\[2.2ex]
   ...
   $\dfrac{1}{\sqrt{t}}$ & $\dfrac{\sqrt{\pi}}{\sqrt{s}} $\\[2.2ex]
   $\sqrt{t}$ & $\dfrac{\sqrt{\pi}}{2 s \sqrt{s}}$\\[2ex]
   $\dfrac{e^{-a t}}{\sqrt{t}}$ & $\dfrac{\sqrt{\pi}}{\sqrt{s+a}}$\\[2.2ex]
   ...
   $\sin(\omega t + \alpha)$ & $\dfrac{\omega\cos\alpha+s\sin\alpha}{s^2+\omega^2}$\\[3.5ex]
   $\cos(\omega t + \alpha)$ & $\dfrac{s\cos \alpha-\omega\sin\alpha}{s^2+\omega^2}$\\[3.5ex]
   ...
   $\dfrac{\sin(2\sqrt{\omega t})}{\sqrt{\omega}}$ & $\dfrac{\sqrt{\pi}e^{-\omega/s}}{s\sqrt{s}}$\\[3.5ex]
   $\dfrac{\cos(2\sqrt{\omega t})}{\sqrt{t}}$ & $\dfrac{\sqrt{\pi}e^{-\omega/s}}{\sqrt{s}}$\\[3.5ex]
...
\begin{longtable}{cc}
   ...
   \input{Imatges/Cap-Laplace-Funcio-14.pdf_tex} & \raisebox{0.8cm}{$K\dfrac{\dfrac{\pi}{\tau}}{s^2+\dfrac{\pi^2}{\tau^2}}\dfrac{(1+e^{-\tau s})}{(1-e^{-\tau s})}$}\\[3.5ex]
   \input{Imatges/Cap-Laplace-Funcio-15.pdf_tex} & \raisebox{0.8cm}{$K\dfrac{\dfrac{\pi}{\tau}}{s^2+\dfrac{\pi^2}{\tau^2}}\dfrac{1}{(1-e^{-\tau s})}$}\\[2.5ex]
    \bottomrule[1pt]
\end{longtable}
...
\begin{exemple}[\CalcTransfLaplace{}]
    ...
    \[\begin{split}
        f_1(t) &= 3t (\varepsilon_0(t) - \varepsilon_1(t)) + \left(-\frac{3}{2}t
        +\frac{9}{2}\right) (\varepsilon_1(t) -
        \varepsilon_3(t))  \\
        &=
    ...
    en el temps un valor d'1 i 3 respectivament; per tant, si apliquem la propietat de la translació, veiem que llurs transformades
    seran $e^{-s}/s^2$  i $e^{-3s}/s^2$
    ...
    \[
        ...
        \hat{U}\sin\dfrac{\pi}{\tau}t,  & 0 \leq t \leq \tau \\ 0, & t > \tau \end{cases}
    \]
    ...
    \[
        u_1(t)=\left(\hat{U}\sin\frac{\pi}{\tau}t\right)(\varepsilon_0(t)-\varepsilon_\tau(t))
    ...
    \[
        ...
        \frac{\hat{U} (\pi/\tau)}{s^2 + (\pi/\tau)^2} e^{-\tau s} =
        \frac{\hat{U} (\pi/\tau)}{s^2 + (\pi/\tau)^2} (1+e^{-\tau s})
    \]
    ...
    \[
        ...
        \frac{\hat{U} (\pi/\tau) }{s^2 + (\pi/\tau)^2}\,\frac{1+e^{-\tau s}}{1-e^{-\tau
        s}}
    \]
...
\begin{exemple}[\ResCircRC{}]
    ...
    A continuació, dividim numerador i denominador per $R C$ per obtenir una expressió que es trobi en la taula
    ...
    Utilitzant la taula \ref{taula:Trans-Laplace-Fun}, obtenim la transformada inversa de Laplace de $I(s)$:
...
\end{center}
    ...
    $i\ped{L}(0)$ per llurs valors numèrics:
    \begin{align*}
    ...
    fraccions parcials; comencem doncs per calcular les arrels del
...
\hyperlink{exemple:CircuitLaplaceNul}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:CircuitLaplaceNul}	
    ...
    $R$, $L$ i $C$, tenim:
    \[
        U\ped{C}(s) = \frac{\sqrt{2}\times 110\times 10^6 s}{(s^2 + 1000 s + 500000)(s^2 + (100\pi)^2)}
    \]
    ...
    500000)(s^2 + (100\pi)^2)}   \\[2ex] &= \frac{C_1 (s+500) + D_1
    ...
    \begin{alignat*}{2}
        ...
        {s^2 +(100\pi)^2}\right|_{s=\complexnum{-500+j500}} &&= \complexnum{-358,57 -j 240,35}\\[3.5ex]
        D_2 + j\,C_2 &= \left.\frac{1}{100\pi}\times\frac{\sqrt{2}\times 110\times 10^6 s}
        {s^2 + 1000 s + 500000}\right|_{s=j 100\pi} &&= \complexnum{188,16 + j240,35}
    \end{alignat*}
    ...
    \begin{dingautolist}{202}
        ...
        \item Per començar, premem la tecla \includegraphics{HPPrime-CAS.pdf} \textit{computer algebra system}, per posar la calculadora en el mode de resolució simbòlic; a continuació premem la tecla \includegraphics{HPPrime-Tool.pdf} i escollim \textsfs{Inverse Laplace} (funció \textsfs{ilaplace}).
         ...
         \item Premem ara la tecla \includegraphics{HPPrime-Enter.pdf}. La variable \textsfs{t} queda substituïda per la variable \textsfs{X} i es crea la funció \textsfs{F1(X)}.
