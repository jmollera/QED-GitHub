...
Un altre IDE d'accés lliure és l'editor  Visual Studio Code\index{Visual Studio Code} (\href{https://code.visualstudio.com/}{https:/\!\!/code.visualstudio.com/}).
Aquest editor pot servir per a diferents llenguatges de programació, i perquè sigui útil per a Python, cal descarregar i instaŀlar  extensions pròpies per a aquest llenguatge.

Un tercer IDE és PyCharm\index{PyCharm} (\href{https://www.jetbrains.com/pycharm/}{https:/\!\!/www.jetbrains.com/pycharm/}); la major part de la funcionalitat és d'accés lliure, i només les característiques més avançades són de pagament. És una mica més complicat de configurar que Spyder, però és més potent i ofereix fins i tot integració amb el sistema de control de versions Git/GitHub.
...
\dirtree{%
.1 \faFolder[regular] Python.
.2 \faFolder[regular] auxiliars \DTcomment{\begin{minipage}[t]{11cm} Carpeta que conté programes utilitzats per crear fitxers de dades, els quals es fan servir en l'elaboració de diversos gràfics del llibre{.} \end{minipage}}.
.2 \faFolder[regular] exemples\_resolts \DTcomment{\begin{minipage}[t]{11cm} Carpeta que conté  els programes del capítol \ref{chap:python-exemples}{.} \end{minipage}}.
.2 \faFolder[regular] qed \DTcomment{\begin{minipage}[t]{11cm} Carpeta que conté  els  programes del capítol \ref{chap:python-programes}{.} \end{minipage}}.
.2 \faFolder[regular] test \DTcomment{\begin{minipage}[t]{11cm} Carpeta que conté  programes per fer proves automatitzades dels programes del capítol \ref{chap:python-programes}, fent servir el mòdul \texttt{unittest}{.} \end{minipage}}.
.2 \faFile[regular] exemples.py \DTcomment{\begin{minipage}[t]{11cm} Fitxer que conté els exemples de la secció \ref{sec:exemples-qed}{.} \end{minipage}}.
.2 \faFile[regular] versions.py \DTcomment{\begin{minipage}[t]{11cm} Fitxer que conté el programa de la secció \ref{sec:python-versions}{.} \end{minipage}}.
}
