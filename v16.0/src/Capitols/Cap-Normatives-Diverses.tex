...
\begin{multicols}{2}
	\begin{list}{}
		...
		en un equip, per  permetre que una seqüència d'operació
		continuï, o per  parar-la, o per proporcionar una prova de la
		...
		actua per igualar i mantenir la velocitat o la  freqüència
		...
		\textbf{Dispositiu controlador de temperatura}. És el que actua per apujar la
		...
		que actua per connectar o desconnectar l'excitació del camp
		d'una màquina.
		...
		un element, accionat manualment, que efectua la transferència dels circuits de control, per  modificar el procés d'operació d'equips de connexió o d'altres dispositius.
		...
		opera en un circuit per  curtcircuitar-lo  o
		...
		vapor, etc.,  a una màquina motriu, per arrencar-la, mantenir-ne
		la  velocitat o parar-la.
		
		\item[\textbf{66}] \index{relé!de passos}  \index{funció de protecció!66}
		\textbf{Relé de passos}. És el que actua per permetre un nombre especificat d'operacions d'un dispositiu
		...
		condicions; pot cooperar també amb altres dispositius, per
		...
		el que actua per escollir automàticament entre certes fonts
		...
		actua per  regular una magnitud, com ara la tensió, el corrent, la potència,
		...
		que actua per augmentar o disminuir el valor de l'excitació
		d'una màquina.
		
		\item[\textbf{94}] \index{relé!de dispar o dispar lliure}  \index{funció de protecció!94}
		\textbf{Relé de disparament o disparament lliure}. És el que actua per disparar o permetre disparar un
...
\begin{center}
	\input{Imatges/Cap-Normatives-Diverses-50-50TD-51.pdf_tex}
...
\begin{list}{}
	...
	\item[$I$] Corrent que circula per la protecció 51. S'expressa en A.\footnote{\label{fn:I-Ip}De fet, només cal que $I$ i $I\ped{P}$ estiguin expressats en les mateixes unitats (per exemple en kA), perquè el  que cal és que  $I/I\ped{P}$ sigui adimensional.}
...
Els valors per defecte de la resta de paràmetres són: $C=\qty{0}{s}$, $B=\qty{0}{s}$, i $T=1$; els fabricants de relés de protecció poden assignar  rangs de valors a aquests paràmetres per obtenir diverses famílies de corbes.\index{CEI!60255-151} En la taula \vref{taula:CEI-51} es poden veure els valors de $k$ i $\alpha$, i els noms usuals que es donen a les corbes d'actuació.
...
Els valors per defecte de la resta de paràmetres són: $B=\qty{0}{s}$, i $T=1$; els fabricants de relés de protecció poden assignar  rangs de valors a aquests paràmetres per obtenir diverses famílies de corbes.\index{IEEE!C37.112} En la taula \vref{taula:IEEE-51} es poden veure els valors de $k$, $C$ i $\alpha$, i els noms que es donen a les corbes d'actuació.
...
\begin{equation}\label{eq:50-51}
	\int_0^\tau \frac{1}{t_{51}(I(t))}  \diff t = 1
\end{equation}

En aquesta equació, $I(t)$ és un corrent variable en el temps, i $\tau$ és el temps que triga a actuar la protecció. Expressat en paraules, podem dir que la protecció actua quan l'àrea sota la corba $1/t_{51}(I(t))$ és igual a 1; d'altra banda, si aquesta àrea no arriba mai a fer-se igual a 1, la protecció no actuarà.

Quan l'equació \eqref{eq:50-51} s'ha de resoldre manualment, usualment es fa servir la integració pel mètode dels trapezis (vegeu la secció \vref{sec:trapezis}). En aquest cas tenim, amb $t_1 = 0$ i $t_n = \tau$:
...
\begin{exemple}[\ProtInvIntVar{} \hyperlink{exemple:ProtInvIntVar}{\large\textcolor{NavyBlue}{(\faPython)}}]\label{ex:ProtInvIntVar}
	...
	\begin{center}
		\input{Imatges/Cap-Normatives-Diverses-ProtInvIntVar.tex}
	\end{center}
...
lletres suplementàries opcionals al final, per poder afegir una informació concreta. Els valors
...
\begin{list}{}
	...
	\item[B] Interruptors automàtics específicament concebuts per ser selectius en condicions de curtcircuit amb altres dispositius de protecció situats en sèrie  aigües avall. Aquests interruptors han d'especificar llur corrent admissible de curta durada $I\ped{cw}$.
\end{list}
...
Es relacionen a continuació diverses normes de l'IEEE \textit{Institute of Electrical and Electronics Engineers}, agrupades per llur àmbit d'aplicació. Podeu trobar el llistat complet de les normes IEEE a l'adreça: \href{https://ieeexplore.ieee.org/Xplore/home.jsp}{https:/\!\!/ieeexplore.ieee.org/Xplore/home.jsp}.
