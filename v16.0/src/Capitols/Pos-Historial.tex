...
\begin{itemize}
    \item En el mòdul \texttt{qed.utils} s'ha modificat la implementació  de la classe \texttt{Complex}, sense canviar-ne cap característica externa; s'ha inclòs la possibilitat de definir els símbols del separador del módul i l'argument,  del grau sexagesimal, i  del radian, afegint a la classe les variables \texttt{angle\_symbol}, \texttt{degree\_symbol} i \texttt{rad\_symbol}.
    ...
    \item En el mòdul \texttt{qed.eng\_elc} s'ha modificat la implementació  de la classe \texttt{MutualCoupling}, eliminat la restricció de que el valor de l'acoblament fos un nombre complex sense part real.
     Per fer evident aquesta circumstància, s'ha canviat el nom de la variable de la classe de \texttt{X} a  \texttt{ZM}; això afecta a l'ús extern d'aquesta classe.
