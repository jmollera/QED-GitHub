...
En l'apartat Notació, s'inclou una nota per indicar  que el valor de l'angle $\psi$ ha d'expressar-se sempre en radian, perquè el valor de $e^{j\psi}$ sigui correcte.
...
Es millora l'exposició de la secció \ref{sec:pot-inst-act-react}.

S'indica en la secció \ref{sec:connex-index-horari} que no són possibles tots els índexs horaris dels transformadors trifàsics.

Es corregeix l'exemple \ref{ex:IccTrafos}, ja que en el cas del transformador YNyn s'havia fet servir un índex horari que no era possible. Addicionalment, es millora la representació de les figures de l'exemple, afegint explícitament la connexió a terra dels generadors.
...
\begin{itemize}
    \item En el mòdul \texttt{qed.utils}, s'ha modificat la implementació  de la classe \texttt{Complex}, sense canviar-ne cap característica externa; s'ha inclòs la possibilitat de definir els símbols del separador del módul i l'argument,  del grau sexagesimal, i  del radian, afegint a la classe les variables \texttt{angle\_symbol}, \texttt{degree\_symbol} i \texttt{rad\_symbol}.
     S'han creat les subclasses \texttt{ComplexR} i \texttt{ComplexD} per manipular nombres complexos polars en radian i en graus, directament, sense la necessitat del paràmetre \texttt{in\_rad} de la classe pare \texttt{Complex}.
    ...
    \item En el mòdul \texttt{qed.eng\_elc}, s'ha modificat la implementació  de la classe \texttt{Network}, afegint a la funció \texttt{\_\_init\_\_} els membres  \texttt{ideal\_Z=1} i \texttt{ideal\_Y=1}; un cop instanciada la classe, aquests valors es poden modificar. El motiu d'aquest canvi és donar la flexibilitat a l'usuari d'escollir altres valors per a \texttt{ideal\_Z} i \texttt{ideal\_Y}, si considera que serien més adequats en relació a les magnituds de la resta de valors  dels components de la xarxa. Donat que la representació interna de la classe \texttt{Network} ha canviat, els mètodes \texttt{load} i  \texttt{save} no són compatibles amb les versions anteriors.
