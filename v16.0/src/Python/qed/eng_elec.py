...
# Two dictionaries of copper cables impedances.
#
# Key: section in mm²
...
#        resistivity variation with the temperature at 20 °C, in 1/°C
...
def z_cable(section: int, length: float, cable: CableType = CableType.A) -> complex:
    ...
    cable:   CableType.A → use dictionary _Z_CABLE_A,
             CableType.V → use dictionary _Z_CABLE_V,
             The default is CableType.A.

    Returns
    -------
    The cable impedance at 90 °C, Ω.
...
def z_series(z: ArrayLike) -> complex:
    ...
    Examples
    --------
    >>> print(z_series([2, 3, 4]))
    9
    >>> print(z_series([2+2j, 3+3j, 4+4j]))
    (9+9j)
    """
    return np.sum(z)
...
def z_parallel(z: ArrayLike) -> complex:
    ...
    Examples
    --------
    >>> print(z_parallel([3, 3, 3]))
    1.0
    >>> print(z_parallel([3+3j, 3+3j, 3+3j]))
    (1+1j)
    """
    p = np.prod(z)
    return p / np.sum(p / z)
...
def apparent_power(u: ArrayLike | complex, i: ArrayLike | complex) -> complex:
    ...
    Returns
    -------
    The apparent power drawn by the load.

    Examples
    --------
    >>> print(apparent_power([24+10j, 26+9j, 27+7j], [1+1j, 2+2j ,3+3j]))
    (206-108j)
    >>> print(apparent_power(110, 3))
    330
    """
    return np.vdot(i, u)
...
def D_to_Y(z_ab: complex, z_bc: complex, z_ca: complex) -> tuple[complex, complex, complex]:
    """
    Convert three delta-connected impedances into three star-connected impedances.
    ...
    Returns
    -------
    Equivalent star-connected impedances: z_an, z_bn, and z_cn.
...
def Y_to_D(z_an: complex, z_bn: complex, z_cn: complex) -> tuple[complex, complex, complex]:
    """
    Convert three star-connected impedances into three delta-connected impedances.
    ...
    Returns
    -------
    Equivalent delta-connected impedances: z_ab, z_bc, and z_ca.
...
class VoltageSource(Element):
    ...
    Z: complex | None = None  # Series impedance. Use None (default) for an ideal source (Z=0)
...
class Network:
    """
    Solve an electrical network using the nodes method.
    ...
    """
    __slots__ = ['name', 'ideal_Z', 'ideal_Y', '_net_branches', '_net_couplings',
                 '_is_solved', '_is_complex', '_extra_nodes', '_extra_branches',
                 '_list_of_nodes', '_list_of_branches', '_list_of_coupled_branches',
                 '_ZN', '_VN', '_IB']

    def __init__(self, name: str = '', ideal_Z: float = 1, ideal_Y: float = 1) -> None:
        """Create an AC or DC network

        Parameters
        ----------
        name:    Name of the network. Default is ''.
        ideal_Z: Impedance to use in ideal voltage sources, and short circuits. Default is 1.
        ideal_Y: Admittance to use in ideal current sources. Default is 1.
        """
        self.name = name
        self.ideal_Z = ideal_Z
        ...
        for branch, (element, (from_node, to_node)) in self._net_branches.items():
            match element:
                ...
                case VoltageSource(E, None):  # Ideal voltage source
                    fill_matrices(branch, from_node, num_nodes_plus_extra, Zb=self.ideal_Z, Eb=E)
                    fill_matrices(num_branches_plus_extra, num_nodes_plus_extra, to_node, Zb=-self.ideal_Z)
                ...
                case CurrentSource(J, None):  # Ideal current source
                    fill_matrices(branch, from_node, to_node, Zb=1 / self.ideal_Y, Jb=J)
                    fill_matrices(num_branches_plus_extra, from_node, to_node, Zb=-1 / self.ideal_Y)
                    num_branches_plus_extra -= 1
                case CurrentSource(J, Y):
                    fill_matrices(branch, from_node, to_node, Zb=1 / Y, Jb=J)
                case ShortCircuit():
                    fill_matrices(branch, from_node, num_nodes_plus_extra, Zb=self.ideal_Z)
                    fill_matrices(num_branches_plus_extra, num_nodes_plus_extra, to_node, Zb=-self.ideal_Z)
                    num_branches_plus_extra -= 1
                    num_nodes_plus_extra -= 1
    ...
    def load(self, filename: str) -> bool:
        ...
        try:
            with open(filename, 'rb') as fh:
                self.name = pickle.load(fh)
                self.ideal_Z = pickle.load(fh)
                self.ideal_Y = pickle.load(fh)
