...
print("I_N = {0:.1f} A".format(I_1_N))
print("I_arr = {0:.1f} A,  I_arr/I_N = {1:.1f}".format(I_arr, I_arr/I_1_N))
...
print("s_T_m,max = {0:.3f},  n_T_m,max = {1:.2f} r/min,  ω_T_m,max = {2:.2f} rad/s".format(s_T_m_max, n_T_m_max, ω_T_m_max))
print("T_m,max = {0:.1f} N·m,  T_m,max/T_m,N = {1:.2f}".format(T_m_max, T_m_max/T_m_N))
print()

n = np.linspace(0, n_m_sinc-0.00001, 100)
I = abs(I_1(s=1-n/n_m_sinc))
T = T_m(s=1-n/n_m_sinc)
try:
    with open("Cap-Motors-Induccio-Ex4.txt", "w", encoding="utf8") as fh:
