...
impedància nuŀla per dues impedàncies en sèrie i de valor
...
\begin{exemple}[Aplicació del mètode dels nusos -- Xarxa amb acoblaments magnètics]\label{ex:metode-nusos}
   ...
   \]
    ...
    Resoldrem a continuació el sistema d'equacions lineals $\mcmplx{Y}\ped{N} \mcmplx{V}\ped{N} = \mcmplx{J}\ped{N}$ amb la calculadora \emph{HP Prime}.\index{HP Prime@\emph{HP Prime}}

    En l'exemple \vref{ex:malles} hem utilitzat l'aplicació \texttt{\textbf{Linear Solver}} per resoldre el sistema d'equacions lineals que s'hi plantejava;  aquesta aplicació només pot resoldre sistemes de dues o tres equacions, i per tant no serveix en aquest cas on en tenim quatre.

    En aquest exemple utilitzarem la funció \texttt{\textbf{RREF}} «Reduced-Row Echelon Form» per resoldre el nostre sistema d'equacions lineals; els passos a seguir són els següents:
    ...
    \begin{dingautolist}{'312}
         ...
         \item A continuació premem dos cops la combinació de tecles  \fbox{\texttt{Shift}} \fbox{\texttt{5}} (\texttt{\textbf{[ ]}}); la calculadora crea un matriu buida, la qual omplirem amb la matriu $\mcmplx{Y}\ped{N}$ seguida pel vector $\mcmplx{J}\ped{N}$, creant una única matriu de quatre files i cinc columnes.

             \includegraphics{Cap-ResXarxElec-Exemple-Circuit-HPP2.png}

         \item Finalment, premem la tecla \fbox{\texttt{Enter}} i la calculadora ens dóna una matriu que conté la solució; les quatre primeres columnes formen una matriu identitat, la qual cosa indica que el sistema té solució i que és única, i la cinquena columna és la  solució buscada.
...
El cas que hem vist fins ara és el més general de tots els
...
és a dir $\betaup\equiv\deltaup$, en aquest cas l'admitància entre
