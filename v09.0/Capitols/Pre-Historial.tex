...
En el capítol dedicat a la transformada de Laplace, es modifiquen segons \cite{SCH} algunes definicions  i s'amplien les taules de transformades de Laplace segons \cite{RASd} i \cite{SCH}.
...
Es millora la definició de l'angle $\alpha$ d'un fasor en l'apartat «Notació».
...
Es canvien els noms de les variables utilitzades en la definició d'un fasor en l'apartat «Notació».
...
Es millora l'explicació de la definició d'un fasor en l'apartat «Notació».
...
Es modifica en tot el llibre la manera de representar una variable acompanyada de les seves unitats. Les variables se separaran de les seves unitats mitjançant el símbol de divisió «/», enlloc de tancar les unitats entre «[» i  «]»; per exemple, enlloc
$S [mm^2]$, a partir d'ara escriurem $S / mm^2$.
...
S'amplien algunes seccions i exemples del llibre, afegint-hi una  resolució numèrica mitjançant la calculadora \emph{HP Prime};\index{HP Prime@\emph{HP Prime}}  aquesta calculadora disposa d'un emulador per a PC, que pot descarregar-se de la pàgina de Hewlett-Packard: \href{http://www.hpprime.de/en/category/6-downloads}{www.hpprime.de/en/category/6-downloads}. Les seccions i els exemples afectats són els següents:
\begin{itemize}
  \item L'exemple \ref{ex:corrent-pic} (Corrent de pic de curtcircuit).
  \item L'exemple \ref{ex:malles} (Resolució de xarxes amb el mètode de les malles).
  \item L'exemple \ref{ex:res-circ-pot} (Resolució de circuits coneixent la potència absorbida).
  \item La secció \ref{sec:calcul-comp-sim} (Components simètriques)
  \item L'exemple \ref{ex:fourier} (Sèries de Fourier).
  \item L'exemple \ref{ex:laplace} (Transformada de Laplace).
  \item L'exemple \ref{ex:metode-nusos} (Resolució de xarxes utilitzant el mètode dels nusos).
  \item La secció \ref{sec:sis_eq_no_lin} (Flux de càrregues).

\end{itemize}

S'amplia la secció \ref{sec:millman} dedicada al teorema de Millman, afegint-hi un exemple més al final.

Es millora l'explicació de la secció \ref{sec:ccRL}.

Es crea la secció \ref{sec:metode-malles} dedicada a la resolució de xarxes elèctriques, utilitzant el mètode de les malles.

S'amplia la secció \ref{sec:escales-log} dedicada a les escales logarítmiques, afegint-hi al final un nou apartat, dedicat a la determinació del paràmetres de funcions que prenen la forma d'una recta en gràfiques d'escala logarítmica--logarítmica.

Es millora l'explicació de l'exemple \ref{ex:comp-sim}.

Es crea la secció \ref{sec:calcul-comp-sim} on es descriuen diversos programes de la calculadora
\emph{HP Prime}\index{HP Prime@\emph{HP Prime}} relacionats amb les components simètriques.

S'amplia l'exemple \ref{ex:fourier}, afegint-hi al final  una nova gràfica.

S'amplia el capítol \ref{chap:resistencies}, afegint-hi la codificació del coeficient de variació amb la temperatura de les resistències, i la norma CEI que defineix les sèries de resistències estàndard.

Es modifiquen les capçaleres de totes les taules del capítol \ref{sec:tr_mes_prot}, ja que els percentatges d'error de tensions i corrents que s'hi indicaven, estaven referits incorrectament als valors nominals dels transformadors.

Es modifiquen les equacions que fan referència a les figures  \ref{pic:tr-pot-esquema-equiv} i \ref{pic:tr-pot-esquema-equiv-complex}, perquè es vegi millor la seva correspondència.


S'afegeixen algunes normes en la secció \ref{sec:normes_IEC}.



En l'apèndix \ref{sec:alf-grec} dedicat a l'alfabet grec, s'utilitza el D.R.A.E.
...
S'amplia l'apèndix \ref{sec:SI}, afegint-hi al final una secció dedicada al factors de conversió d'unitats.

Es revisa la taula \ref{taula:SI-altres-experimentals} i l'apèndix \ref{sec:const_fis}, utilitzant les publicacions de l'any 2014 del «Committee on Data for Science and Technology» (CODATA).\index{CODATA}


S'amplia la secció \ref{sec:llei-s-c-t}, afegint-hi les equacions de les coordenades del baricentre d'un triangle, i es modifica de manera corresponent la figura \ref{pic:llei-s-c-t}.

Es modifiquen les figures \ref{pic:metode-newton} i \ref{pic:metode-secant}.
