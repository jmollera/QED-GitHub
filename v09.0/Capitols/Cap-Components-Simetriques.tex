...
Per tant, expressant els fasors del sistema asimètric en funció
dels fasors dels tres sistemes simètrics, tenim:
\begin{subequations}
\begin{align}
...
\begin{exemple}[Aplicació de les components simètriques]\label{ex:comp-sim}
    ...
    \begin{center}
        \input{Imatges/Cap-CompSim-Tensions-Exemple.pdf_tex}
    \end{center}
    ...
    té component homopolar; a més, el sistema de tensions fase--neutre
    tampoc no en tindrà, ja que la càrrega trifàsica és equilibrada
    (tres resistències idèntiques).

    Trobem a continuació les components directa i inversa de les
    ...
    Podem ara ja calcular la potència consumida per la càrrega
    ...
    \cmplx{I}\ped{A,2}^*  = \\
    &= 0 + 3 \times \SIpd{1308,91}{-24,96}{V} \times
    ...
    \begin{align*}
        \cmplx{U}\ped{AG} &= 0 + \SIpd{1308,91}{-24,96}{V} +
        \SIpd{311,64}{8,34}{V}  =
...
\index{HP Prime@\emph{HP Prime}} per tal de facilitar la resolució numèrica de les equacions que han aparegut en aquest capítol.
...
\bfseries
    ...
    BEGIN
      LOCAL a:=(-1/2,\textsurd{}3/2);
      [[1,1,1],[1,a^2,a],[1,a,a^2]]*v;
    END;
\end{alltt}

La funció \texttt{\textbf{ABC→A012}} és la inversa de la funció anterior, i utilitza l'equació \eqref{eq:c_sim_mat1} per obtenir els tres fasors
...
\bfseries
    ...
    BEGIN
      \textsurd{}3*[v(1)*exp(\textpi{}*i/6),v(2)*exp(-\textpi{}*i/6)];
    END;
\end{alltt}
...
\bfseries
    ...
    BEGIN
      [v(1)/exp(\textpi{}*i/6),v(2)/exp(-\textpi{}*i/6)]/\textsurd{}3;
    END;
\end{alltt}
...
La funció \texttt{\textbf{FF→FG}} obté els fasors de les tres tensions fase--neutre $\cmplx{U}\ped{AG}$, $\cmplx{U}\ped{BG}$ i $\cmplx{U}\ped{CG}$, que tenen el punt neutre en el baricentre del triangle format pels fasors de  les tres tensions fase--fase
...
\bfseries
    ...
    // Triangle de tensions:[AB,BC,CA] → Fasors de tensió:[AB,BC,CA]
    // AB es pren com a fasor de referència
    BEGIN
      LOCAL \textphi{},mode_AAngle;
      mode_AAngle:=AAngle;
      AAngle:=1; // Radiants
      \textphi{}:=Triangle_Solver.SSS(v(1),v(3),v(2));
      AAngle:=mode_AAngle;
      [v(1),v(2)*exp(i*(\textpi{}+\textphi{}(2))),v(3)*exp(i*(\textpi{}-\textphi{}(3)))];
    END;
\end{alltt}
