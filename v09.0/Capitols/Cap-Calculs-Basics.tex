...
tota la xarxa, i tantes tensions base com nivells de tensió
...
la impedància de la màquina en qüestió estarà expressada ja directament en per unitat.
...
Per tal de calcular la impedància base $Z_{\text{B}_\text{M}}$ per convertir $\cmplx{Z}\ped{M}$ en un valor en per unitat, no podem utilitzar les equacions \eqref{eq:bases_pu}, ja que cadascuna de les dues branques de l'acoblament magnètic està a una tensió diferent; en aquest cas cal utilitzar l'equació que es por trobar en \cite{TLE}:
\begin{equation}
...
Per tal de  calcular l'admitància base $Y_{\text{B}_\text{M}}$ per convertir $\cmplx{Y}\ped{M}$ en un valor en per unitat, no podem utilitzar les equacions \eqref{eq:bases_pu}, ja que cadascuna de les dues branques de l'acoblament capacitiu està a una tensió diferent; en aquest cas cal utilitzar l'equació que es por trobar en \cite{TLE}:
\begin{equation}
...
\begin{exemple}[Resolució d'un circuit coneixent la potència que absorbeix]
    Resoldre el circuit de la dreta de la Figura \vref{pic:EZS}, donats el següents valors en per unitat:\label{ex:res-circ-pot}
    \[
    ...
    Resoldrem a continuació aquest exemple amb la calculadora \emph{HP Prime},\index{HP Prime@\emph{HP Prime}} seguint els passos següents:
    \begin{dingautolist}{'312}
        \item Començarem escrivint un programa que servirà per resoldre tan el cas de corrent continu com el de corrent altern. A aquest programa l'anomenarem \texttt{EZS→U}.

        El program prendrà com a dades els valors  $E$, $R$ i $P$, o els valors  $\cmplx{E}$, $\cmplx{Z}$ i $\cmplx{S}$, i calcularà el valor  $U$, o el valor $\cmplx{U}$.

        Es presenta a continuació el llistat del programa que cal introduir en la calculadora:

    \begin{alltt}
    \bfseries
    EXPORT EZS→U(E,Z,S)
    // Tensió:E, Impedància:Z, Potència:S → Tensió:U
    BEGIN
      LOCAL ReU,ImU,ZcS;
      ZcS:=CONJ(Z)*S;
      ImU:=IM(ZcS)/ABS(E);
      ReU:=POLYROOT(\{1,-ABS(E),RE(ZcS)+ImU^2\});
      IF TYPE(ReU(1))==3 THEN
    ...
    \item A continuació, suposant que la calculadora està en el mode \texttt{RPN}, entrem els valors de $\cmplx{E}$, $\cmplx{Z}$ i $\cmplx{S}$: \texttt{(0.4,0.3)} \fbox{\texttt{Enter}} \texttt{(0,0.1)} \fbox{\texttt{Enter}} \texttt{(0.6,0.45)} \fbox{\texttt{Enter}}, i a continuació escrivim el nom del programa: \texttt{EZS→U()}.

        \includegraphics{Cap-CalcBas-EZS-HPP1.png}

        Si la calculadora estigués en els modes \texttt{Algebraic} o \texttt{Textbook}, escriuríem directament: \texttt{EZS→U(0.4+0.3*i,0.1*i,0.6+0.45*i)}.

    \item Finalment, premem la tecla \fbox{\texttt{Enter}} i la calculadora ens dóna el valor de la tensió $\cmplx{U}$.
...
\subsection{Determinació dels paràmetres d'una funció representada com una recta}\index{escales logarítmiques!determinació dels paràmetres d'una funció representada com una recta}
...
\begin{exemple}[Càlcul de les constants $n$ i $k$ en una escala logarítmica--logarítmica]
    ...
    Partint del valor d'abscises $x_1=1$, veiem que el valor corresponent d'ordenades és: $y_1=2$, i per tant tenim directament:
    \[
        k = 2
    \]
