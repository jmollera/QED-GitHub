...
Capítol \ref{chap:nusos} per tal de resoldre la xarxa.
...
Ara bé, en la majoria dels casos es compleix: $|\cmplx{\theta}\ped{c}| \ll 1$, i utilitzant els desenvolupaments en sèrie de les funcions $\sinh$ i $\tanh$, al voltant de 0, tenim:
\begin{align}
...
\begin{exemple}[Control de tensió d'un nus amb condensadors]\label{ex:control-v-cond}
    ...
    recalcular la xarxa, passant el nus 2 a ser un nus de càrrega, i
...
\begin{exemple}[Control de tensió d'un nus amb un transformador]
    ...
    possibles, i tornar a recalcular la xarxa passant la tensió del nus
...
En primer lloc, cal escriure una funció en un «fitxer M» que representi el sistema d'equacions no lineals que es vol resoldre; utilitzant la variable \texttt{x(1)} per a $|\cmplx{v}_2|$ i la variable \texttt{x(2)} per a $\delta_2$, creem un fitxer amb el contingut següent:
...
\begin{dingautolist}{'312}
    ...
    \item En primer lloc premem la tecla \fbox{\texttt{Apps}} i seleccionem l'aplicació \texttt{\textbf{Solve}}.
    ...
    \item Tot seguit premem la tecla \fbox{\texttt{Num}} i entrem els valors inicials de les variables. Al camp \texttt{\textbf{V}} entrem: \texttt{1.05}, i al camp \texttt{\textbf{D}} entrem: \texttt{0}.
