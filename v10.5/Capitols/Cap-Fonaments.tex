...
\begin{exemple}[Càrrega i descàrrega d'un circuit R-L]\label{ex:carrega-descarrega-RL}
    ...
    \begin{dingautolist}{'312}
        \item En primer lloc premem la tecla \includegraphics{HPPrime-Apps.pdf} i seleccionem l'aplicació \textsf{Function}.

             \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP1.png}

        \item Tot seguit entrem en els camps \textsf{F1(X)}, \textsf{F2(X)} i \textsf{F3(X)} les equacions de $i_1(t)$, $i_2(t)$ i $i_3(t)$; enlloc dels valors \num{1,7293} i  \num{0,4558}, utilitzarem \textsf{F1(0.02)} i \textsf{F2(0.04)} respectivament, per tal que la calculadora els calculi automàticament. Com a variable utilitzarem \textsf{X} enlloc de \textsf{t}.

            En el camp \textsf{F4(X)} entrem la funció $i(t)$, amb l'ajut de la funció  \textsf{Heaviside(X)}. Escriurem:\break \textsf{F1(X) * (Heaviside(X-0)-Heaviside(X-0.02)) + F2(X) * (Heaviside(X-0.02)-Heaviside(X-0.04))\break + F3(X) * Heaviside(X-0.04)}.

            Finalment, deixem marcat el camp \textsf{F4(X)} i desmarquem els camps \textsf{F1(X)}, \textsf{F2(X)} i \textsf{F3(X)}.

            \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP2.png}

            Una altra manera de crear el camp \textsf{F4(X)}, és usant la funció \textsf{IFTE(Cond,ST,SF)}; aquesta funció avalua l'expressió \textsf{Cond}, si el resultat   és cert executa \textsf{ST} i si és fals executa \textsf{SF}. Podem escriure per tant: \textsf{IFTE(X<0.02,F1(X),IFTE(X<0.04,F2(X),F3(X)))}.

            \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP3.png}

        \item Ara ja podem dibuixar la funció $i(t)$.

            Comencem ajustant els paràmetres de la gràfica, prement les tecles \includegraphics{HPPrime-Shift.pdf} \includegraphics{HPPrime-Plot.pdf}; fixem els dos valors de \textsf{X Rng} a \textsf{0} i \textsf{0.1} respectivament, els dos valors de \textsf{Y Rng} a \textsf{0} i \textsf{2} respectivament, el valor de \textsf{X Tick} a \textsf{0.01}, i el valor de \textsf{Y Tick} a \textsf{0.25}.
...
\begin{exemple}[Corrent de pic asimètric]\label{ex:corrent-pic}
    ...
    \index{HP Prime!exemples} Els passos a seguir són els següents:
     ...
     \begin{dingautolist}{'312}
        ...
        \item Tot seguit entrem en els camps \textsf{F1(X)} i \textsf{F2(X)} les equacions de $i\ped{ac}(t)$ i $i\ped{dc}(t)$ que hem calculat en l'exemple \vref{ex:cc-RL}; com a variable utilitzarem \textsf{X} enlloc de \textsf{t}.

            En el camp \textsf{F3(X)} entrem \textsf{F1(X)+F2(X)}.

            Finalment, deixem marcat el camp \textsf{F3(X)} i desmarquem els camps \textsf{F1(X)} i \textsf{F2(X)}. D'aquesta manera la calculadora només dibuixarà la funció total $i(t)$, i no les parcials $i\ped{ac}(t)$ i $i\ped{dc}(t)$.
        ...
        \item Ara ja podem dibuixar la funció del corrent total de curtcircuit.
            ...
            Comencem ajustant els paràmetres de la gràfica, prement les tecles \includegraphics{HPPrime-Shift.pdf} \includegraphics{HPPrime-Plot.pdf}; fixem els dos valors de \textsf{X Rng} a \textsf{0} i \textsf{0.1} respectivament, els dos valors de \textsf{Y Rng} a \textsf{-40000} i \textsf{80000} respectivament, el valor de \textsf{X Tick} a \textsf{0.01}, i el valor de \textsf{Y Tick} a \textsf{10000}.
        ...
        \item Premem a continuació el botó \hpbutton{\phantom{x}Fcn\phantom{x}} i seleccionem la funció \textsf{Extremum} del menú que apareix.
...
\begin{exemple}[Aplicació del mètode de les malles]\label{ex:malles}
    ...
    \begin{dingautolist}{'312}
     \item En primer lloc premem la tecla \includegraphics{HPPrime-Apps.pdf} i seleccionem l'aplicació \textsf{Linear Solver}.
