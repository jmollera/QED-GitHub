...
\begin{exemple}[Resolució d'un circuit coneixent la potència que absorbeix]
    ...
    Resoldrem a continuació aquest exemple amb la calculadora \emph{HP Prime},\index{HP Prime!exemples}\index{HP Prime!programes!EZSU@\texttt{EZS\_U}} seguint els passos següents:
    \begin{dingautolist}{'312}
        \item Començarem escrivint un programa, que anomenarem  \textsf{EZS\_U}, el qual servirà per resoldre tan el cas de corrent continu com el de corrent altern;  es pot veure el seu llistat en la secció \vref{sec:HP_ELC}.
    ...
    \item A continuació, suposant  que la calculadora està en el mode \textsf{RPN}, entrem els valors de $\cmplx{E}$, $\cmplx{Z}$ i $\cmplx{S}$: \textsf{(0.4,0.3)} \includegraphics{HPPrime-Enter.pdf} \textsf{(0,0.1)} \includegraphics{HPPrime-Enter.pdf} \textsf{(0.6,0.45)} \includegraphics{HPPrime-Enter.pdf}, i  escrivim després el nom del programa: \textsf{EZS\_U()}.

        \includegraphics{Cap-CalcBas-EZS-HPP1.png}

    \item Premem a continuació la tecla \includegraphics{HPPrime-Enter.pdf} i la calculadora ens dóna el valor de la tensió $\cmplx{U}$.

        \includegraphics{Cap-CalcBas-EZS-HPP2.png}


     \item   Si la calculadora estigués en el mode \textsf{Algebraic} o en el mode  \textsf{Textbook}, escriuríem directament: \textsf{EZS\_U(0.4+0.3*i,0.1*i,0.6+0.45*i)}, i després de prémer la tecla \includegraphics{HPPrime-Enter.pdf} la calculadora ens donaria el valor de la tensió $\cmplx{U}$.
