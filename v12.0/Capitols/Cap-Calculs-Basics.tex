...
xarxa, i com a potència base la potència nominal d'un dels transformadors o generadors de la xarxa; també és usual utilitzar com a potència base el valor \SI{100}{MVA}.
...
\begin{align}
    ...
    x\ped{cc}\right)}\qquad\qquad(\text{amb }U=U\ped{T1})\\[1ex]
    I\ped{F} &= \frac{S\ped{T}}{\sqrt{3} U\ped{T2}
    x\ped{cc}}\qquad\qquad(\text{amb }U=U\ped{T1}\text{ i }
    S\ped{cc}=\infty)
\end{align}
...
\begin{exemple}[Corrent de curtcircuit en el secundari d'un transformador]
    ...
    $U=\SI{6900}{V}$, $U\ped{T1}=\SI{6900}{V}$,
    $U\ped{T2}=\SI{400}{V}$, $S\ped{T}=\SI{850}{kVA}$ i
