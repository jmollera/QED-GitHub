...
Tots els programes d'aquest llibre estan escrits en Python 3, i s'han executat utilitzant la distribució Anaconda 2023.03-1. Aquesta distribució fa servir la llibreria numba per tal d'accelerar els càlculs d'altres llibreries, com ara pandapower; també utilitza la llibreria mkl (\textit{Math Kernel Library})  d'Intel®.

La llibreria pandapower ha d'instaŀlar-se executant des de la consola  l'ordre: \texttt{pip install pandapower}. Les llibreries qed.utils i qed.eng\_elc són l'objecte del capítol \ref{chap:python-programes}. 
