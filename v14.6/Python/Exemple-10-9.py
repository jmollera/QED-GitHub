...
s_fun = mot.s_match(T_load, *font_ext)
mot_s_fun = mot.run(s_fun, *font_ext)
mot_s_arr = mot.run(1, *font_ext)
print(f's,fun = {s_fun:.4f},   n_m_fun = {mot.to_n_m(s=s_fun):.1f} r/min')
print(f'U_1,fun = {ComplexD(mot_s_fun.U):.3f} V')
print(f'I_1,fun = {abs(mot_s_fun.I):.1f} A')
print(f'T_m,fun = {mot_s_fun.T_m:.1f} N·m\n')
print(f'U_1,arr = {ComplexD(mot_s_arr.U):.3f} V')
print(f'I_1,arr = {abs(mot_s_arr.I):.1f} A')
print(f'T_m,arr = {mot_s_arr.T_m:.1f} N·m\n')

s_T_m_max = mot.s_T_m_max(Z_sist+Z_cable)
n_T_m_max = mot.to_n_m(s=s_T_m_max)
ω_T_m_max = mot.to_ω_m(s=s_T_m_max)
T_m_max = mot.run(s_T_m_max, *font_ext).T_m
print(f's_T_m,max = {s_T_m_max:.4f}')
print(f'n_T_m,max = {n_T_m_max:.1f} r/min')
print(f'T_m,max = {T_m_max:.1f} N·m\n')

mot_arrencada = mot.start_up(T_load, J, *font_ext)
print(f't_arr = {mot_arrencada.t_start:.1f} s')

n_m = np.linspace(0, mot.n_m_sync, 500)  # r/min
s = mot.to_s(n_m=n_m)
mot_s = mot.run(s, *font_ext)
...
ax1.plot(n_m, mot_s.T_m, 'red', label=r'$T_{\mathrm{m}}$')
ax2 = ax1.twinx()
ax2.set_ylim(0, 75)
ax2.set_yticks(np.arange(0, 76, 5))
ax2.set_ylabel(r'$I_1$ / A')
ax2.plot(n_m, abs(mot_s.I), 'orange', label=r'$I_1$')
...
ax.plot(n_m, abs(mot_s.U), 'red', label=r'$U_1$')
...
mot_arr = mot.run(mot_arrencada.s_start_up(t_arr), *font_ext)
...
ax.set_ylabel(r'$t$ / s')
ax.plot(np.abs(mot_arr.I), t_arr, 'red')
plt.show()
...
ax.plot(t_arr, np.abs(mot_arr.U), 'red')
plt.show()
