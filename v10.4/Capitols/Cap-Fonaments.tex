...
\begin{exemple}[Càrrega i descàrrega d'un circuit R-L]\label{ex:carrega-descarrega-RL}
    ...
    \begin{dingautolist}{'312}
        ...
        \item Tot seguit entrem en els camps \texttt{\textbf{F1(X)}}, \texttt{\textbf{F2(X)}} i \texttt{\textbf{F3(X)}} les equacions de $i_1(t)$, $i_2(t)$ i $i_3(t)$; enlloc dels valors \num{1,7293} i  \num{0,4558}, utilitzarem \texttt{\textbf{F1(0.02)}} i \texttt{\textbf{F2(0.04)}} respectivament, per tal que la calculadora els calculi automàticament. Com a variable utilitzarem \texttt{\textbf{X}} enlloc de \texttt{\textbf{t}}.

            En el camp \texttt{\textbf{F4(X)}} entrem la funció $i(t)$, amb l'ajut de la funció  \texttt{\textbf{Heaviside(X)}}. Escriurem: \texttt{\textbf{F1(X) * (Heaviside(X-0)-Heaviside(X-0.02)) + F2(X) * (Heaviside(X-0.02)-\newline Heaviside(X-0.04)) + F3(X) * Heaviside(X-0.04)}}.

            Finalment, deixem marcat el camp \texttt{\textbf{F4(X)}} i desmarquem els camps \texttt{\textbf{F1(X)}}, \texttt{\textbf{F2(X)}} i \texttt{\textbf{F3(X)}}.

            \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP2.png}

            Una altra manera de crear el camp \texttt{\textbf{F4(X)}}, és usant la funció \texttt{\textbf{IFTE(Cond,ST,SF)}}; aquesta funció avalua l'expressió \texttt{\textbf{Cond}}, si el resultat   és cert executa \texttt{\textbf{ST}} i si és fals executa \texttt{\textbf{SF}}. Podem escriure per tant: \texttt{\textbf{IFTE(X<0.02,F1(X),IFTE(X<0.04,F2(X),F3(X)))}}.

            \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP3.png}

        \item Ara ja podem dibuixar la funció $i(t)$.

            Comencem ajustant els paràmetres de la gràfica, prement les tecles \fbox{\texttt{Shift}} \fbox{\texttt{Plot}} (\texttt{\textbf{Plot Setup}}); fixem els dos valors de \texttt{\textbf{X Rng}} a \texttt{\textbf{0}} i \texttt{\textbf{0.1}} respectivament, els dos valors de \texttt{\textbf{Y Rng}} a \texttt{\textbf{0}} i \texttt{\textbf{2}} respectivament, el valor de \texttt{\textbf{X Tick}} a \texttt{\textbf{0.01}}, i el valor de \texttt{\textbf{Y Tick}} a \texttt{\textbf{0.25}}.

            \includegraphics{Cap-Fonaments-RL-Carrega-Descarrega-HPP4.png}

        \item A continuació premem la tecla \fbox{\texttt{Plot}} i la calculadora dibuixa la gràfica.
    ...
    \index{HP Prime!exemples} Els passos a seguir són els següents:
     ...
     \begin{dingautolist}{'312}
        ...
        \item Tot seguit entrem en els camps \texttt{\textbf{F1(X)}} i \texttt{\textbf{F2(X)}} les equacions de $i\ped{ac}(t)$ i $i\ped{dc}(t)$ que hem calculat en l'exemple \vref{ex:cc-RL}; com a variable utilitzarem \texttt{\textbf{X}} enlloc de \texttt{\textbf{t}}.
            ...
            Finalment, deixem marcat el camp \texttt{\textbf{F3(X)}} i desmarquem els camps \texttt{\textbf{F1(X)}} i \texttt{\textbf{F2(X)}}. D'aquesta manera la calculadora només dibuixarà la funció total $i(t)$, i no les parcials $i\ped{ac}(t)$ i $i\ped{dc}(t)$.
