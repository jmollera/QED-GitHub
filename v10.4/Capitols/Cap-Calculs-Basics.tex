...
\begin{exemple}[Resolució d'un circuit coneixent la potència que absorbeix]
    ...
    \item A continuació, suposant que la calculadora està en el mode \texttt{RPN}, entrem els valors de $\cmplx{E}$, $\cmplx{Z}$ i $\cmplx{S}$: \texttt{(0.4,0.3)} \fbox{\texttt{Enter}} \texttt{(0,0.1)} \fbox{\texttt{Enter}} \texttt{(0.6,0.45)} \fbox{\texttt{Enter}}, i  escrivim després el nom del programa: \texttt{EZS\_U()}.

        \includegraphics{Cap-CalcBas-EZS-HPP1.png}

    \item Premem a continuació la tecla \fbox{\texttt{Enter}} i la calculadora ens dóna el valor de la tensió $\cmplx{U}$.

        \includegraphics{Cap-CalcBas-EZS-HPP2.png}
        

     \item   Si la calculadora estigués en els modes \texttt{Algebraic} o \texttt{Textbook}, escriuríem directament: \texttt{EZS\_U(0.4+0.3*i,0.1*i,0.6+0.45*i)}, i després de prémer la tecla \fbox{\texttt{Enter}} la calculadora ens donaria el valor de la tensió $\cmplx{U}$.
