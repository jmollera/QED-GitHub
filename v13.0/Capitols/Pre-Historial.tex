...
Es modifica lleugerament el nom del llibre, passant a dir-se \textit{Qüestions Electrotècniques Diverses} en lloc de \textit{Qüestions Diverses d'Electrotècnia}, i per tant a parir d'ara es podrà denominar amb l'acrònim «QED» (\emph{Quod Erat Demonstrandum}).

Es canvia la tipografia dels exemples utilitzada en les versions anteriors, passant ara a ser escrits en lletra recta en lloc d'en lletra inclinada.
...
Es modifica la secció 10.4.2, afegint-hi les equacions del rendiment i de les potències aparent i reactiva en funció del lliscament, i ampliant l'exemple 10.4.
...
S'estandarditza l'ús dels subíndexs «N» i  «n» en tot el llibre. A partir d'ara s'utilitzarà  «N» per a  «neutre», i  «n» per a   «nominal».
...
Es reordenen les seccions del capítol \ref{ch:fonaments}.

En les figures de l'exemple \ref{ex:teorema-superpos}, s'indiquen com a quantitats complexes les impedàncies de la inductància i del condensador.

Es crea el nou exemple \ref{ex:v-mitj-v-ef}.

Es millora la Figura \ref{pic:Millman} afegint-hi la lletra $\alphaup$ que faltava  a baix a l'esquerra.

Es millora la informació representada en la figura \ref{pic:val-mitja}.

Es revisa el capítol \ref{sec:laplace} per actualitzar la simbologia utilitzada en la representació de la transformada de Laplace.

Es revisa la secció \ref{sec:resistencies-colors} per adaptar la codificació en colors de les resistències a l'edició de l'any 2016 de la norma CEI 60062 \textit{Marking codes for resistors and capacitors}.

Es revisa la secció \ref{sec:resistencies-series} per adaptar les sèries de resistències estandarditzades a l'edició de l'any 2015 de la norma CEI 60063 \textit{Preferred number series for resistors and capacitors}.

S'amplia l'exemple \ref{ex:resistencies-colors}.

Es corregeixen les unitats dels exemples \ref{ex:cap-termica-cable} i \ref{ex:mm2-a-AWG}.

S'afegeixen les unitats que faltaven en el text i els exemples del capítol \ref{chap:motors-ind}. 

Dins del capítol \ref{chap:motors-ind} es crea la nova secció \ref{sec:motors-50-60} per tractar la compatibilitat entre els motors de \qty{50}{Hz} i els de \qty{60}{Hz}.

Es millora la representació i l'explicació de l'equació \eqref{eq:Tload-U1-Yeq}.

S'afegeixen les unitats que faltaven en el text i els exemples del capítol \ref{chap:nusos}.

Es millora l'explicació dels paràmetres de l'equació \eqref {eq:corba-50-51}.

En la  secció \ref{sec:codi-IK}, es canvia la norma de referència del codi IK, passant de l'antiga EN 50102 a l'actual CEI 62262 \textit{Degrees of protection provided by enclosures for
electrical equipment against external mechanical impacts (IK Code)}.

En la secció \ref{sec:normes_IEC}, s'afegeixen les normes CEI 60062,  CEI 60063,  CEI 60529 i  CEI 62262.

En els apèndixs  \ref{sec:SI} i \ref{sec:const_fis}, es modifiquen les unitats de la constant de Planck, passant de \unit{J.s} a \unit{J/Hz}; això es fa d'acord amb  la publicació de l'any 2018 del \textit{Committee on Data for Science and Technology} (CODATA).

En la secció \ref{sec:unitata-derv-SI}, s'indica la relació numèrica entre el grau Celsius i el kelvin.

S'amplia la secció \ref{sec:normes-escript}, dedicada a les normes d'escriptura de quantitats i unitats.


S'explica en l'apèndix \ref{sec:const_fis}, dedicat a les constants físiques,  el canvi introduït en les definicions de l'any 2018 d'algunes constants respecte de les definicions del 2014, quan diverses constants  van passar de tenir valors mesurats amb una certa precisió a tenir valors definits exactes, i a l'inrevés.

Es revisa l'índex alfabètic, al final del llibre, eliminant entrades sobreres, afegint-ne algunes i coŀlocant-ne d'altres en la posició correcta.
