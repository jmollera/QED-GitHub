...
\begin{exemple}[Aplicació de les components simètriques -- Impedàncies equilibrades]\label{ex:comp-sim}
	\addcontentsxms{Aplicació de les components simètriques -- Impedàncies equilibrades}
    ...
    resistències, aplicant les lleis de Kirchhoff; donat que les tres resistències són idèntiques, les seves  components    directa, inversa i homopolar $R_1$,  $R_2$ i $R_0$ també ho són i tenen el mateix valor de $\qty{10}{\ohm}$.
    \begin{align*}
    \cmplx{I}\ped{A,1} &=
...
\begin{exemple}[Aplicació de les components simètriques -- Impedàncies desequilibrades]\label{ex:comp-sim-deseq}
	\addcontentsxms{Aplicació de les components simètriques -- Impedàncies desequilibrades}
...
\begin{itemize}
   \item \funsfbs{Triangle\_a\_Fasors}. A partir d'un triangle de tensions obté els fasors que el formen.
