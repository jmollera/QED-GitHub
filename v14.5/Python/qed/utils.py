...
class ComplexR(complex):
    ...
    def __new__(cls, *args):
        ...
        try:
            match args:
                ...
                case [mod, arg_rad]:
                    return super().__new__(cls, cmath.rect(float(mod), float(arg_rad)))
                case _:
                    raise TypeError
        except TypeError:
            print('Only 1 or 2 parameters allowed.',
                  ' 1 parameter:  int, float, complex, str',
                  ' 2 parameters: module and argument in radians',
                  sep='\n')
            return None
    ...
    def __format__(self, fmt=''):
        ...
        if (pos := fmt.find('/')) == -1:
            ...
            fmt_mod = fmt[:pos]
            fmt_arg = fmt[pos + 1:]
        return ''.join([format(self.mod, fmt_mod), '∠', format(self.arg, fmt_arg)])
...
class ComplexD(complex):
    """
    Class to handle polar complex numbers in degrees.
        ...
        try:
            match args:
                case [z]:
                    return super().__new__(cls, z)
                case [mod, arg_deg]:
                    return super().__new__(cls, cmath.rect(float(mod), math.radians(float(arg_deg))))
                case _:
                    raise TypeError
        except TypeError:
            print('Only 1 or 2 parameters allowed',
                  '  1 parameter:  int, float, complex or str',
                  '  2 parameters: module and argument in degrees',
                  sep='\n')
            return None
