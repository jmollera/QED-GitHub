...
try:
    ...
    print(err)
# try:
#     with open("Cap-Motors-Induccio-CaractMotor-Aux-2.txt", "w", encoding="utf8") as fh:
#         for i in range(len(n)):
#             fh.write("{0:8.3f} {1:6.3f} {2:6.3f} {3:6.3f} {4:6.3f}\n".format(n[i], Pm[i], P[i], Q[i], S[i]))
# except OSError as err:
#     print(err)
# try:
#     with open("Cap-Motors-Induccio-CaractMotor-Aux-3.txt", "w", encoding="utf8") as fh:
#         for i in range(len(n)):
#             fh.write("{0:8.3f} {1:8.6f} {2:8.6f}\n".format(n[i], cos_φ[i], η[i]))
# except OSError as err:
#     print(err)
