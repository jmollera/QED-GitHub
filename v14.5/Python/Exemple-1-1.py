...
circuit = ee.Network()
circuit.add(ee.VoltageSource(from_to=(0, 1), E=U_1, Z=R_1), branch=1)
circuit.add(ee.VoltageSource(from_to=(0, 1), E=U_2, Z=R_2), branch=2)
circuit.add(ee.VoltageSource(from_to=(0, 1), E=U_3, Z=R_3), branch=3)
circuit.solve()
...
print(f'Y_No = {Y_No:.2f} S')
print(f'J_No = {J_No/1000:.2f} kA')
circuit.add(ee.Impedance(from_to=(1, 0), Z=R_Q), branch=4)
circuit.solve()
I_Q = circuit.current(branch=4)
U_Q = circuit.voltage(branch=4)
print(f'I_Q  = {I_Q:.4f} A')
print(f'U_Q  = {U_Q:.2f} V\n')
