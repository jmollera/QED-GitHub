...
S'ha millorat la representació de les figures de l'exemple \ref{ex:IccTrafos}, afegint explícitament la connexió a terra dels generadors.

S'ha actualitzat la distribució de Python utilitzada per executar els programes d'aquest llibre; els canvis més significatius són l'ús de la versió 3.13 de Python, el pas de la versió 1.x a la 2.x de la llibreria \texttt{numpy}, i el pas de la versió 2.x a la 3.x de la llibreria \texttt{pandapower}.
A causa del canvi de versió de la llibreria \texttt{pandapower}, ha calgut modificar el programa \texttt{Exemple-12-3.py} afegint el nou paràmetre \texttt{tap\_changer\_type='Ratio'} a la funció \texttt{create\_transformer\_from\_parameters}, perquè funcionés correctament.

S'ha canviat la implementació  de la classe \texttt{Complex} sense modificar-ne cap característica externa.

S'ha canviat la implementació  de les classes \texttt{IECcurve}, \texttt{IEEEcurve} i \texttt{Speed}, canviant-ne els noms del elements interns de minúscula a majúscula, per emfatitzar que són constants. Això afecta a l'ús extern d'aquestes classes.
