...
\section*{Versió 15.1 (\today)}
\addcontentsline{toc}{section}{Versió 15.1}

S'unifica la font utilitzada per a les lletres gregues a la \texttt{Euler}; és la mateixa  que es fa servir per a les expression i variables matemàtiques.
...
\begin{itemize}
    \item En el mòdul \texttt{qed.utils}, s'ha modificat la implementació  de la classe \texttt{Complex}, sense canviar-ne cap característica externa.
    ...
    \item Ha calgut modificar el programa \texttt{Exemple-12-3.py}, afegint el paràmetre \texttt{tap\_changer\_type} amb el valor \texttt{'Ratio'} a la funció \texttt{create\_transformer\_from\_parameters}. Això és degut al canvi de versió de la llibreria \texttt{pandapower}.
\end{itemize}

A la secció \ref{sec:bit-octet-byte} s'indica que els prefixes binaris «robi» i «quebi»  s'han inclòs en la revisió de la norma CEI 80000-13:2025, i, en conseqüència,  ja són d'ús oficial.
