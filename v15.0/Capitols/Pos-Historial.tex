...
\begin{itemize}
	...
	\item Llistat 16.24. S'amplia aquest programa, fent ús de la classe \texttt{Motor3ph}, per tal de calcular el temps que triga el motor a arrencar, al 100 \% i al 80 \% de la tensió nominal, i per representar l'evolució temporal del corrent d'arrencada a aquestes dues tensions.	
	\item  Llistat 16.25. S'amplia aquest programa, fent ús de la classe \texttt{Motor3ph}, per tal de calcular el temps que triga el motor a arrencar, i per representar l'evolució temporal del  corrent d'arrencada i de la tensió en el motor.
\end{itemize}
...
S'amplia la secció \ref{sec:pot-inst-act-react}.

S'afegeixen les unitats que faltaven en els exemples \ref{ex:CarDescRL}, \ref{ex:CurtcircuitRL}, \ref{ex:CircuitFourier}, \ref{ex:CircuitLaplace} i \ref{ex:CircuitLaplaceNul}.

S'amplia la secció \ref{sec:prod-convolucio}  per descriure les propietats commutativa, distributiva i associativa, del producte de convolució.

Es crea la taula \ref{taula:AWG-k} per millorar la presentació de la informació que en edicions anteriors s'exposava d'una manera diferent.

Es milloren les figures \ref{fig:Hom-YNy} i \ref{fig:Hom-Yyn}.

Es modifica la figura \ref{pic:mot-T-vel-mot-load} i el text previ que l'acompanya, per fer referència al punt de funcionament del conjunt motor--càrrega, en lloc de  les condicions nominals.


Es crea la secció \ref{sec:xarxes-curtcircuits} on s'explica el mètode de la superposició aplicat al càlcul de curtcircuits en xarxes elèctriques.

Es crea la secció \ref{sec:progs-llibre-internet} per indicar on es poden obtenir els fitxers dels programes Python descrits en aquest llibre.

En el capítol \ref{chap:python-programes} es millora la programació del mòdul  \texttt{qed.eng\_elec}:
\begin{itemize}
	...
	\item Es milloren els mètodes \texttt{\_\_str\_\_} i \texttt{results} de la classe \texttt{Network}, per tal de donar una informació més completa dels nusos i les branques de la xarxa.
...
A la secció \ref{sec:SI-intro}, s'indica que la publicació més recent del \textit{Bureau 	International des Poids et Mesures} del Sistema Internacional d'unitats, és la  9a edició de març de 2019, revisada l'agost de 2024 per millorar la descripció dels angles i de les 
magnituds unitàries. Els canvis queden restringits a la taula \ref{sec:unitats-derv-SI}.

S'amplia la secció \ref{sec:bit-octet-byte}. S'incorporen a la taula \ref{taula:prefix-inform} els prefixes binaris «robi» i «quebi», els quals s'inclouran en la pròxima revisió de la norma CEI 80000-13:2024, actualment en estat d'esborrany.

S'actualitza l'apèndix  \ref{sec:const_fis} amb els nous valors recomanats de les constants físiques  l'any 2022 pel \textit{Committee on Data for Science and Technology} (CODATA). S'han afegit constants a la taula \ref{taula:Const-Fis}.

Es reordena el contingut de la secció \ref{sec:lleis-trig-triang}.

Es crea la secció \ref{sec:trig-grafics} per tal de representar les gràfiques de les funcions trigonomètriques: sin, cos, tan, csc, sec i cot, i de les funcions hiperbòliques: sinh, cosh, tanh, csch, sech i coth.

Es crea la secció \ref{sec:HPPrime-Intro} incloent-hi el text que en edicions anteriors estava a l'inici de l'apèndix \ref{sec:progs-HP}; al final d'aquesta secció s'afegeix la indicació d'on es poden obtenir els fitxers dels programes de la calculadora \textsf{HP Prime} descrits en les seccions següents d'aquest apèndix.
