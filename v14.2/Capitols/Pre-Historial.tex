...
Es modifiquen els noms de les funcions de l'apèndix F, per tal d'igualar-les amb els noms de les funcions del mòdul Python \funsfbs{qed.eng\_elc} del capítol 15.
...
En l'exemple \ref{ex:CircuitFourier} i en la secció  \ref{sec:sis_eq_no_lin}, s'indica l'equivalència entre els programes MATLAB® i GNU Octave.

S'afegeix la nota «a» a la taula \ref{taula:altres-angleses}.

Es canvien algunes equacions de la secció \ref{sec:mot-tens-pot-red}.

Es modifiquen els dibuixos dels exemples \ref{ex:FluxCarrXarxa}, \ref{ex:ControlTensCond} i \ref{ex:ControlTensTrafo}, representant en els nusos flotants una xarxa externa, en lloc d'un generador.

La secció \ref{sec:ieee-c37-2} es divideix en dues subseccions, i s'amplia amb l'explicació de l'actuació de la funció  51 quan el corrent que circula és variable en el temps. S'afegeixen les unitats que faltaven en l'equació \eqref{eq:corba-50-51}, i en les taules \ref{taula:CEI-51} i \ref{taula:IEEE-51}.

S'afegeixen algunes normes en la secció \ref{sec:normes_IEC}.

En la secció \ref{sec:py-desenvel-prog}, s'inclou la descripció dels IDE Visual Studio Code i PyCharm.

En la secció \ref{sec:python-libs}, s'inclou l'explicació de la llibreria \funsfbs{pandapower}.


En el capítol \ref{chap:python-programes}, es fan les modificacions següents al mòdul  \funsfbs{qed.eng\_elec}:
\begin{itemize}
	\item S'afegeix la funció \funsfbs{rms}, que calcula el valor eficaç d'una funció periòdica.
	\item S'afegeix la funció \funsfbs{average}, que calcula el valor mitjà d'una funció periòdica.
	\item S'afegeix l'excepció \funsfbs{NetworkNotSolvable}, associada a la classe \funsfbs{Network}.
	\item Es canvia el nom de la classe \funsfbs{Admitance} per \funsfbs{Admittance}.
	\item Es millora la programació d'algunes funcions sense variar-ne la funcionalitat.
\end{itemize}


En el capítol \ref{chap:python-exemples}, s'afegeix la resolució amb Python dels exemples \ref{ex:CircuitLaplaceNul}  i  \ref{ex:ProtInvIntVar}, i els exemples \ref{ex:FluxCarrXarxa}, \ref{ex:ControlTensCond} i \ref{ex:ControlTensTrafo} es resolen  fent servir addicionalment la llibreria \funsfbs{pandapower}.

En la taula \ref{taula:SI-prefixes}, s'inclouen els  nous prefixos de l'SI per als factors $10^{27}$,  $10^{30}$,  $10^{-27}$ i  $10^{-30}$, introduïts en la 27a \textit{Conférence Générale des Poids et Mesures} de l'any 2022.

Es millora la presentació de les taules \ref{taula:prefix-inform} i \ref{taula:rev-min}.
