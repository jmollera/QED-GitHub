...
de Newton-Raphson. \index{Newton-Raphson} Aquí, no obstant això, no
...
\section{Resolució de sistemes d'equacions no lineals amb els programes Mathematica®, MATLAB® i GNU Octave,  i amb la calculadora HP Prime}
...
La resolució d'aquest sistema d'equacions no lineals amb el programa Mathematica® és molt senzilla, ja que la funció \funsfbs{FindRoot} ens proporciona directament la solució; utilitzant la variable \mathsfbs{v_2} per a $|\cmplx{v}_2|$ i la variable \mathsfbs{\delta_2} per a $\delta_2$, tenim:
...
La resolució amb el programa MATLAB® s'obté a partir de la funció  \funsfbs{fsolve}; per  poder utilitzar aquesta funció cal tenir instaŀlada l'extensió del programa \textit{Optimization toolbox}. La resolució amb el programa GNU Octave és idèntica; en aquest cas la funció \funsfbs{fsolve} està disponible de sèrie, sense necessitat de carregar cap mòdul addicional.

En primer lloc, cal escriure una funció en un «fitxer M» que representi el sistema d'equacions no lineals que es vol resoldre; utilitzant la variable \funsfbs{x(1)} per a $|\cmplx{v}_2|$ i la variable \funsfbs{x(2)} per a $\delta_2$, creem el fitxer «SistEqNoLin.m» amb el contingut següent:

\hspace{1cm}\funsfbs{function y = SistEqNoLin(x)}
...
\hspace{1cm}\funsfbs{>{}> fsolve(@SistEqNoLin, [1.05; 0.0])}

\hspace{1cm}\funsfbs{ans =}

\hspace{1cm}\funsfbs{\phantom{ans --}0.970306}

\hspace{1cm}\funsfbs{\phantom{ans }--0.060222}

\subsection{Resolució amb la calculadora \textsf{HP Prime}}
\index{HP Prime@\textsf{HP Prime}!exemples}

La resolució d'aquest sistema d'equacions no lineals amb la calculadora \textsf{HP Prime} és molt senzilla, ja que pot fer-se utilitzant l'aplicació integrada \funsfbs{Solve}. Els passos a seguir són els següents:

\begin{dingautolist}{'312}

    \item En primer lloc, premem la tecla \includegraphics{HPPrime-Apps.pdf} i seleccionem l'aplicació \funsfbs{Solve}.
    ...
    \item Tot seguit premem la tecla \includegraphics{HPPrime-Num.pdf} i entrem els valors inicials de les variables. Al camp \funsfbs{V} entrem: \funsfbs{1.05}, i al camp \funsfbs{D} entrem: \funsfbs{0}.
