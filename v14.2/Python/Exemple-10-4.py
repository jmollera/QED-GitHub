...
print(f'n_m,n = {n_m_n:.1f} r/min,  ω_m,n = {ω_m_n:.3f} rad/s\n')
...
cos_𝜑_n = np.cos(np.angle(Z_mot(s=s_n)))
cos_𝜑_arr = np.cos(np.angle(Z_mot(s=1)))
print(f'cos 𝜑,n = {cos_𝜑_n:.4f}')
print(f'cos 𝜑,arr = {cos_𝜑_arr:.4f}\n')

P_n = 3*U_1*I_1_n*cos_𝜑_n
η_n = P_m_n/P_n
print(f'η,n = {η_n:.4f}\n')
...
cos_𝜑 = np.cos(np.angle(Z_mot(s=1-n/n_m_sinc)))
P = 3*U_1*I1*cos_𝜑/1000  # kW
Q = 3*U_1*I1*np.sqrt(1-cos_𝜑**2)/1000  # kvar
S = np.hypot(P, Q)  # kVA
η = Pm/P
...
ax.set_xlabel(r'$n_{\mathrm{m}}$ / r/min')
ax.plot(n, cos_𝜑, 'red', label=r'$\cos\varphi$')
