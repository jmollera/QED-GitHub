...
Es crea una part nova en el llibre --- la IV --- dedicada al llenguatge de programació Python, la qual conté els capítols \ref{chap:python-llenguatge},  \ref{chap:python-programes} i  \ref{chap:python-exemples}. Les parts IV i V de les versions anteriors passen a ser les parts V  i VI  respectivament. 

S'amplia l'apartat Prefaci per descriure els nous capítols \ref{chap:python-llenguatge},  \ref{chap:python-programes} i  \ref{chap:python-exemples}.

S'afegeix el símbol (\faPython) al final del títol dels  exemples del llibre que estan resolts mitjançant Python; aquest símbol és un enllaç que porta directament a l'apartat del capítol \ref{chap:python-exemples} on es resol l'exemple.

En l'apartat Notació, s'unifica el símbol que es fa servir per definir l'angle d'un fasor, utilitzant la lletra $\theta$.

Es millora l'explicació de l'exemple \ref{ex:ValEfMit}.

En la secció \ref{sec:Ti-prot-IEEE}, es clarifica el significat de $Z\ped{ns}$ i de $S\ped{n}$ basant-se en la norma IEEE C37.110 \textit{ Guide for the Application of Current Transformers Used for Protective Relaying Purposes}.

En la secció \ref{sec:ch-xarxes-elec-intro}, s'afegeix el circuit equivalent d'una font de corrent ideal.

S'afegeixen algunes normes en la secció \ref{sec:normes_IEC}.

En la secció \ref{sec:SI-intro}, es posen al dia les adreces d'internet  del \textit{Bureau International des Poids et Mesures}, i s'actualitza el Reial decret que empara l'ús del Sistema Internacional d'unitats.

A la taula \ref{taula:func-trig-signes}, es canvien els símbols < i > pels correctes $\leq$ i $\geq$ respectivament.

Es modifiquen els noms de les funcions de l'apèndix \ref{sec:progs-HP}, per tal d'igualar-les amb els noms de les funcions del mòdul Python \funsfbs{qed.eng\_elc} del capítol \ref{chap:python-programes}.
